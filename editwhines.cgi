...
if ( $cgi->param('update') ) {
    ...
    if ( $cgi->param("add_event") ) {
        ...
        $sth = $dbh->prepare( "INSERT INTO whine_events " . "(owner_userid) " . "VALUES (?)" );
        $sth->execute($userid);
    }
    else {
        for my $eventid ( keys %{$events} ) {
            ...
            if ( $cgi->param("remove_event_$eventid") ) {
                ...
                # events
                $sth = $dbh->prepare( "DELETE FROM whine_events " . "WHERE id=? AND owner_userid=?" );
                $sth->execute( $eventid, $userid );
            }
            else {
                ...
                # add a query
                    ...
                    $sth = $dbh->prepare( "INSERT INTO whine_queries " . "(eventid) " . "VALUES (?)" );
                    $sth->execute($eventid);
                }
            }
