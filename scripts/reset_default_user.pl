...
if ($component) {
    ...
    my $colname
        = $field->name eq 'qa_contact'
        ? 'initialqacontact'
        : 'initialowner';
    ( $component_id, $default_user_id )
        = $dbh->selectrow_array( "SELECT id, $colname FROM components " . "WHERE name = ? AND product_id = ?",
...
# build list of bugs
my $bugs_query = "SELECT bug_id, qa_contact, component_id " . "FROM bugs WHERE product_id = ?";
my @args       = ($product_id);
...
foreach my $bug (@$bugs) {
    ...
    if ($default_user_id) {
        ...
        my $colname
            ...
            ? 'initialqacontact'
            : 'initialowner';
        $new_user_id = $dbh->selectrow_array( "SELECT $colname FROM components WHERE id = ?", undef, $old_comp_id );
    }
    ...
    if ( $old_user_id != $new_user_id ) {
        ...
        $dbh->do( "UPDATE bugs SET " . $field->name . " = ? WHERE bug_id = ?", undef, $new_user_id, $bug_id );
        $dbh->do(
