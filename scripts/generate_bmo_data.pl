...
if ( !$dbh->selectrow_array("SELECT 1 FROM rep_platform WHERE value = 'ARM'") ) {
    ...
    foreach my $platform (@platforms) {
        $dbh->do( "INSERT INTO rep_platform (value, sortkey) VALUES (?, ?)", undef, ( $platform, $count + 100 ) );
    }
}
...
if ( !$dbh->selectrow_array("SELECT 1 FROM op_sys WHERE value = 'AIX'") ) {
    ...
    my $count = 100;
    foreach my $os (@oses) {
        $dbh->do( "INSERT INTO op_sys (value, sortkey) VALUES (?, ?)", undef, ( $os, $count + 100 ) );
    }
}
...
my @users = (
    ...
    map { { login => $_, realname => ( split( /@/, $_, 2 ) )[0], password => '*', } } map {
        map {@$_}
            values %$_
...
my @products = (
    {
        ...
        versions         => [ '34 Branch',  '35 Branch', '36 Branch',  '37 Branch',  'Trunk',      'unspecified' ],
        milestones       => [ 'Firefox 36', '---',       'Firefox 37', 'Firefox 38', 'Firefox 39', 'Future' ],
        defaultmilestone => '---',
        components       => [
            {
                name         => 'General',
...
my $default_op_sys_id   = $dbh->selectrow_array("SELECT id FROM op_sys WHERE value = 'Unspecified'");
my $default_platform_id = $dbh->selectrow_array("SELECT id FROM rep_platform WHERE value = 'Unspecified'");
    ...
    if ( !$new_product ) {
        ...
        $dbh->do(
            'INSERT INTO products (name, description, classification_id,
                  ...
                  VALUES (?, ?, ?, ?, ?)',
            undef,
            ( $product->{product_name}, $product->{description}, $class_id, $default_op_sys_id, $default_platform_id )
        );
...
my @keywords = (
    ...
    {
        name        => 'relnote',
        description => 'This bug need to be put on release notes for next ' . 'milestone announcement.'
    },
);
