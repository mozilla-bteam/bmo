...
my %params;
GetOptions( \%params, 'help|h|?', 'oldid|o=s', 'newid|n=s', 'product|p=s', 'component|c:s', 'doit|d' ) or pod2usage(1);
...
# Get the flag names
my $old_flag_name = $dbh->selectrow_array( "SELECT name FROM flagtypes WHERE id = ?", undef, $params{'oldid'} );
my $new_flag_name = $dbh->selectrow_array( "SELECT name FROM flagtypes WHERE id = ?", undef, $params{'newid'} );

# Find the product id
my $product_id = $dbh->selectrow_array( "SELECT id FROM products WHERE name = ?", undef, $params{'product'} );

# Find the component id if not __ANY__
my $component_id;
if ( $params{'component'} ) {
    $component_id = $dbh->selectrow_array( "SELECT id FROM components WHERE name = ? AND product_id = ?",
