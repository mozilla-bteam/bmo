...
my @missing_versions;
my $ra_versions
    = $dbh->selectcol_arrayref( "SELECT DISTINCT version FROM bugs WHERE component_id = ?", undef, $component->id );
...
my $ra_milestones = $dbh->selectcol_arrayref( "SELECT DISTINCT target_milestone FROM bugs WHERE component_id = ?",
    undef, $component->id );
...
if (@missing_versions) {
    $missing_error
        .= "'$new_product_name' is missing the following version(s):\n  " . join( "\n  ", @missing_versions ) . "\n";
}
...
# Bugs table
$dbh->do( "UPDATE bugs SET product_id = ? WHERE component_id = ?", undef, ( $new_product->id, $component->id ) );
...
# Components
$dbh->do( "UPDATE components SET product_id = ? WHERE id = ?", undef, ( $new_product->id, $component->id ) );
...
sub fix_flags {
    ...
    my $type_ids = $dbh->selectcol_arrayref( "SELECT DISTINCT type_id FROM $table WHERE component_id = ?", undef,
        $component->id );
