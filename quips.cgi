...
if ( $action eq "show" ) {
    ...
    my $quipsref = $dbh->selectall_arrayref("SELECT quipid, userid, quip, approved FROM quips");
...
if ( $action eq 'approve' ) {
    ...
    $dbh->do( "UPDATE quips SET approved = 1 WHERE quipid IN (" . join( ",", @approved ) . ")" ) if ( $#approved > -1 );
    $dbh->do( "UPDATE quips SET approved = 0 WHERE quipid IN (" . join( ",", @unapproved ) . ")" )
        if ( $#unapproved > -1 );
...
if ( $action eq "delete" ) {
    ...
    ( $vars->{'deleted_quip'} ) = $dbh->selectrow_array( "SELECT quip FROM quips WHERE quipid = ?", undef, $quipid );
