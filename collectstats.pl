...
if ( $switch{'regenerate'} ) {
    %bug_resolution = @{ $dbh->selectcol_arrayref( 'SELECT bug_id, resolution FROM bugs', { Columns => [ 1, 2 ] } ) };
    %bug_status     = @{ $dbh->selectcol_arrayref( 'SELECT bug_id, bug_status FROM bugs', { Columns => [ 1, 2 ] } ) };
...
FIN
    ...
    foreach my $data (@data) {
        print DATA join( '|', map { defined $data->{$_} ? $data->{$_} : '' } ( 'DATE', @statuses, @resolutions ) )
            . "\n";
    }
    print DATA ( join '|', @row ) . "\n";
    close DATA;
...
sub regenerate_stats {
    ...
    my $query
        ...
        . qq{ FROM bugs $from_product
                   WHERE } . $dbh->sql_to_days('creation_ts') . qq{ IS NOT NULL $and_product
...
FIN
        ...
        for ( my $day = $start + 1; $day <= $end; $day++ ) {
            ...
            for my $bug (@bugs) {
                my $status = _get_value( $removed->{'bug_status'}->{$bug}, $bug_status, $day, $bug );

                if ( defined $bugcount{$status} ) {
                    $bugcount{$status}++;
                }

                my $resolution = _get_value( $removed->{'resolution'}->{$bug}, $bug_resolution, $day, $bug );
        ...
        my $tend = time;
        print "\rRegenerating $product \[100.0\%] - " . delta_time( $tstart, $tend ) . "\n";
...
sub CollectSeriesData {
    ...
    my $deletesth = $dbh->prepare(
        "DELETE FROM series_data
                                   WHERE series_id = ? AND series_date = " . $dbh->quote($today)
    );
