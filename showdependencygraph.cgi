...
chmod Bugzilla::Install::Filesystem::CGI_WRITE,
    $filename
...
if ( $webdotbase =~ /^https?:/ ) {
    ...
    my ( $pngfh, $pngfilename ) = File::Temp::tempfile(
        "XXXXXXXXXX",
        SUFFIX => '.png',
        DIR    => $webdotdir
    );

    chmod Bugzilla::Install::Filesystem::WS_SERVE,
        $pngfilename
        or warn install_string(
        'chmod_failed',
        {
    ...
    my ( $mapfh, $mapfilename ) = File::Temp::tempfile(
        "XXXXXXXXXX",
        SUFFIX => '.map',
        DIR    => $webdotdir
    );
