...
sub _flag_deferred {
    ...
    if ( !exists $self->{deferred} ) {
        my $dbh = Bugzilla->dbh;
        my ($defer_until)
            = $dbh->selectrow_array( "SELECT defer_until FROM nag_defer WHERE flag_id=?", undef, $self->id );
    ...
    if ($save) {
        ...
        if ( $input->{remove_watched_users} ) {
            ...
            foreach my $login (@$del_watching) {
                ...
                $dbh->do( "DELETE FROM nag_watch WHERE watcher_id=? AND nagged_id=?", undef, $user->id, $u->id );
        ...
        if ( $input->{add_watching} ) {
            ...
            foreach my $login (@$add_watching) {
                ...
                $dbh->do( "INSERT INTO nag_watch(watcher_id, nagged_id) VALUES(?, ?)", undef, $user->id, $u->id );
                $watching{ $u->login } = 1;
            }
        }
