...
sub send_nags {
    ...
    foreach my $report ( @{ $args{reports} } ) {
        ...
        foreach my $request (@$rows) {
            ...
            push @{ $requests->{ $request->{recipient_id} }->{ $target->login }->{$report}->{ $request->{flag_type} } },
                $request;
            push @{ $requests->{ $request->{recipient_id} }->{ $target->login }->{bug_ids}->{$report} },
                $request->{bug_id};
        }
        ...
        foreach my $recipient_id ( keys %$requests ) {
            foreach my $target_login ( keys %{ $requests->{$recipient_id} } ) {
                ...
                # build a summary
                $rh->{summary}->{$report}
                    = join( ', ', map { scalar( @{ $rh->{$report}->{$_} } ) . ' ' . $_ } @{ $rh->{types}->{$report} } );
