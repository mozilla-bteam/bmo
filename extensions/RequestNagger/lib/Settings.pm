...
sub new {
    ...
    foreach my $row (
        @{
            $dbh->selectall_arrayref( "SELECT setting_name,setting_value FROM nag_settings WHERE user_id = ?",
                { Slice => {} }, $user_id )
        }
        )
    {
...
sub set {
    ...
    if ( exists $self->{$field} ) {
        $dbh->do( "UPDATE nag_settings SET setting_value=? WHERE user_id=? AND setting_name=?",
            undef, $value, $self->{user_id}, $field );
    }
    else {
        $dbh->do( "INSERT INTO nag_settings(user_id, setting_name, setting_value) VALUES (?, ?, ?)",
