...
sub revision {
    ...
    unless ( defined $params->{revision}
        && detaint_natural( $params->{revision} ) )
    {
    ...
    my $user = Bugzilla::User->new( { name => AUTOMATION_USER } );
    ...
    # Else bug is private
        ...
        ThrowUserError('invalid_phabricator_sync_groups')
          unless $phab_sync_groups;
        my $sync_group_names = [ split( '[,\s]+', $phab_sync_groups ) ];

        my $bug_group_names = [ map { $_->name } @{ $bug->groups_in } ];
    ...
    my $attachment =
      create_revision_attachment( $bug, $revision_id, $revision_title );
    ...
    return {
        ...
        attachment_link => correct_urlbase()
          . "attachment.cgi?id="
          . $attachment->id
    };
}
