...
sub _fix_negatives {
    my ( $dbh, $field ) = @_;
    my $user_ids = $dbh->selectcol_arrayref( "SELECT userid FROM profiles WHERE ${field}_request_count < 0" );
    return unless @$user_ids;
    $dbh->do( "UPDATE profiles SET ${field}_request_count = 0 WHERE " . $dbh->sql_in( 'userid', $user_ids ) );
