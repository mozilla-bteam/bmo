...
sub _reviewers_objs {
    ...
    if ( !$object->{reviewers} ) {
        my $dbh = Bugzilla->dbh;
        my $user_ids
            = $dbh->selectcol_arrayref( "SELECT user_id FROM ${type}_reviewers WHERE ${type}_id = ? ORDER BY sortkey",
            undef, $object->id, );
        ...
        if ( !$unfiltered ) {
            @reviewers = grep { $_->is_enabled && $_->is_active && $_->name !~ UNAVAILABLE_RE && !$_->reviews_blocked }
                @reviewers;
        }
...
sub _bug_mentors {
    ...
    if ( !$self->{bug_mentors} ) {
        ...
        $self->{bug_mentors} = [ sort { $a->login cmp $b->login } @{ $self->{bug_mentors} } ];
    }
...
sub _bug_check_bug_mentors {
    ...
    my $mentors = [
        grep { !$seen{ $_->id }++ }
        map { Bugzilla::User->check( { name => $_, cache => 1 } ) } ref($value) ? @$value : ($value)
    ];
...
sub _update_user_table {
    ...
    foreach my $old_user (@$old_users) {
        if ( !grep { $_->id == $old_user->id } @$new_users ) {
            $dbh->do( "DELETE FROM $table WHERE $id_field = ? AND user_id = ?", undef, $object->id, $old_user->id, );
    ...
    foreach my $new_user (@$new_users) {
        if ( !grep { $_->id == $new_user->id } @$old_users ) {
            $dbh->do( "INSERT INTO $table ($id_field, user_id) VALUES (?, ?)", undef, $object->id, $new_user->id, );
            push @added, $new_user;
        }
    }
...
sub _check_requestee {
    ...
    if ( $flag->requestee->reviews_blocked ) {
        ThrowUserError( 'reviews_blocked', { requestee => $flag->requestee, flagtype => $flag->type->name } );
    }
}
...
sub _adjust_request_count {
    ...
    $add = $add == -1 ? ' - 1' : ' + 1';
    Bugzilla->dbh->do( "UPDATE profiles SET $field = $field $add WHERE userid = ?", undef, $requestee_id );
...
sub user_preferences {
    my ( $self, $args ) = @_;
    return
        unless $args->{current_tab} eq 'account'
        && $args->{save_changes};
...
sub review_suggestions_report {
    ...
    my $products = [];
    my @products = sort { lc( $a->name ) cmp lc( $b->name ) } @{ Bugzilla->user->get_accessible_products };
...
sub install_update_db {
    my $dbh = Bugzilla->dbh;
    $dbh->bz_add_column( 'products', 'reviewer_required',    { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    $dbh->bz_add_column( 'profiles', 'review_request_count', { TYPE => 'INT2',    NOTNULL => 1, DEFAULT => 0 } );
    $dbh->bz_add_column( 'profiles', 'feedback_request_count', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0 } );
    $dbh->bz_add_column( 'profiles', 'needinfo_request_count', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0 } );
