...
$(function () {
    ...
    YUI.add('bz-review-history', function (Y) {
        ...
        function fetch_flags(user) {
            return new Promise((resolve, reject) => {
                Bugzilla.API.get('review/flag_activity', {
                    type_name: 'review',
                    requestee: user,
                }).then(data => {
        ...
        function fetch_bug_summaries(flags) {
            return new Promise((resolve, reject) => {
                ...
                Bugzilla.API.get('bug', {
                    id: bug_ids,
                    include_fields: ['id', 'summary'],
                }).then(data => {
                    data.bugs.forEach(bug => summary[bug.id] = bug.summary);
        ...
        function fetch_attachment_descriptions(flags) {
            return new Promise((resolve, reject) => {
                ...
                Bugzilla.API.get(`bug/attachment/${attachment_ids[0]}`, {
                    attachment_ids,
                    include_fields: ['id', 'description'],
                }).then(data => {
                    flags.forEach(flag => flag.attachment = data.attachments[flag.attachment_id]);
                    resolve(flags);
                }).catch(error => {
                    reject(error.message);
                });
