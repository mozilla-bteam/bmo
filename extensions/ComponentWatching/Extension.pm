...
sub _generate_watch_user_name {
    ...
    my ($component) = @_;
    return _sanitise_name( $component->name ) . '@' . _sanitise_name( $component->product->name ) . '.bugs';
}
...
sub bugmail_recipients {
    ...
    if (@$diffs) {
        ...
        foreach my $ra (@$diffs) {
            next
                if !( exists $ra->{'old'} && exists $ra->{'field_name'} );
...
sub _getUserWatches {
    ...
    @watches = sort { $a->{'product'}->name cmp $b->{'product'}->name
            || $a->{'component'}->name cmp $b->{'component'}->name } @watches;
...
sub _deleteWatch {
    ...
    $dbh->do( "DELETE FROM component_watch WHERE id=? AND user_id=?", undef, $id, $user->id );
}
...
sub _addDefaultSettings {
    ...
    my @defaultEvents = (
        EVT_OTHER,           EVT_COMMENT,       EVT_ATTACHMENT, EVT_ATTACHMENT_DATA,
        EVT_PROJ_MANAGEMENT, EVT_OPENED_CLOSED, EVT_KEYWORD,    EVT_DEPEND_BLOCK,
        EVT_BUG_CREATED,
    );
...
sub reorg_move_component {
    ...
    Bugzilla->dbh->do( "UPDATE component_watch SET product_id=? WHERE component_id=?",
