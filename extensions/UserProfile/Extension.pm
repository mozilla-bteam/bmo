...
sub _user_set_last_activity_ts {
    ...
    Bugzilla->dbh->do( "UPDATE profiles SET last_activity_ts = ? WHERE userid = ?", undef, $value, $self->id );
...
sub _user_clear_last_statistics_ts {
    ...
    # we update the database directly to avoid audit_log entries
    Bugzilla->dbh->do( "UPDATE profiles SET last_statistics_ts = NULL WHERE userid = ?", undef, $self->id );
    Bugzilla->memcached->clear( { table => 'profiles', id => $self->id } );
}
...
sub _recalc_insert {
    my ( $self, $user ) = @_;
    Bugzilla->dbh->do( "INSERT IGNORE INTO profiles_statistics_recalc SET user_id=?", undef, $user->id );
}

sub _recalc_remove {
    my ( $self, $user ) = @_;
    Bugzilla->dbh->do( "DELETE FROM profiles_statistics_recalc WHERE user_id=?", undef, $user->id );
}
...
sub webservice_user_get {
    ...
    my $timestamps = $dbh->selectall_hashref(
        "SELECT userid,last_activity_ts FROM profiles WHERE " . $dbh->sql_in( 'userid', $ids ), 'userid', );
...
sub install_filesystem {
    ...
    $files->{$script_name} = { perms => Bugzilla::Install::Filesystem::OWNER_EXECUTE };
}
