...
sub bug_check_can_change_field {
    ...
    my ( $bug, $field, $new_value, $old_value, $priv_results ) = @$args{qw(bug field new_value old_value priv_results)};
...
sub bug_start_of_update {
    ...
    my $result      = "There were $num_changes changes to fields on bug $bug_id" . " at $timestamp.";
...
sub flag_end_of_update {
    ...
    # how to use this hook.
    my $flag_params = $args;
    my ( $object, $timestamp, $old_flags, $new_flags ) = @$flag_params{qw(object timestamp old_flags new_flags)};
    ...
    my $bug_id
        ...
        ? $object->id
        : $object->bug_id;
    my $result = "$granted flags were granted and $denied flags were denied" . " on bug $bug_id at $timestamp.";
...
sub product_end_of_create {
    ...
    my $default_assignee = new Bugzilla::User( { name => Bugzilla->params->{maintainer} } );
...
sub sanitycheck_check {
    ...
    while ( my ( $userid, $login, $numgroups ) = $sth->fetchrow_array ) {
        $status->( 'example_check_au_user_alert', { userid => $userid, login => $login }, 'alert' );
        $seen_nonau = 1;
    }
