...
my @METHOD_WHITELIST = (
    'User.get',           'User.login',      'User.valid_login', 'Bug.add_comment',
    'Bug.add_attachment', 'Bug.attachments', 'Bug.get',          'Bug.update_attachment',
);
...
sub template_before_process {
    ...
    return
        unless (
        (      $file =~ /bug\/(show-header|edit).html.tmpl$/
    ...
    if ($bug) {
        if ( $file eq 'attachment/create.html.tmpl' ) {
            if (   $bug->product eq 'Core'
                || $bug->product eq 'Firefox'
