...
my $bugs_query
    = "SELECT distinct bug_id FROM attachments WHERE mimetype='text/x-review-board-request' AND isobsolete=0";
...
foreach my $bug_id (@$bug_ids) {
    ...
    foreach my $summary (@$summaries) {
        ...
        my $attacher = Bugzilla::User->new(
            {
                id    => $parent->{"submitter_bmo_id"},
                cache => 1
            }
        );
        ...
        foreach my $child (@children) {
            ...
            foreach my $reviewer_id ( @{ $child->{"reviewers_bmo_ids"} } ) {
                ...
                if ( defined($flag) ) {
                    ...
                    set_review_flag( $child_attach, $flag_type, $flag->status, $reviewer, $attacher );
            ...
            foreach my $extra_reviewer_id ( keys %tmp_parent_flags ) {
                ...
                set_review_flag( $child_attach, $flag->type, $flag->status, $extra_reviewer, $flag->setter );
            }
