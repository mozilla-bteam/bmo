...
sub install_update_db {
    ...
    if ( !$dbh->bz_column_info( 'profiles', 'comment_count' ) ) {
        $dbh->bz_add_column( 'profiles', 'comment_count', { TYPE => 'INT3', NOTNULL => 1, DEFAULT => 0 } );
    ...
    if ( !$dbh->bz_column_info( 'profiles', 'creation_ts' ) ) {
        ...
        foreach my $ra_row (@$ra) {
            ...
            if ( !$when ) {
                ($when)
                    = $dbh->selectrow_array(
                    "SELECT bug_when FROM bugs_activity WHERE who=? ORDER BY bug_when " . $dbh->sql_limit(1),
                    undef, $user_id );
            }
            if ( !$when ) {
                ($when)
                    = $dbh->selectrow_array(
                    "SELECT bug_when FROM longdescs WHERE who=? ORDER BY bug_when " . $dbh->sql_limit(1),
                    undef, $user_id );
            }
            if ( !$when ) {
                ($when)
                    = $dbh->selectrow_array(
                    "SELECT creation_ts FROM bugs WHERE reporter=? ORDER BY creation_ts " . $dbh->sql_limit(1),
                    undef, $user_id );
            }
            if ( !$when ) {
                $when = $now;
...
sub _update_comment_count {
    ...
    my $id = $self->id;
    my ($count) = $dbh->selectrow_array( "SELECT COUNT(*) FROM longdescs WHERE who=?", undef, $id );
    return if $self->{comment_count} == $count;
    $dbh->do( 'UPDATE profiles SET comment_count=? WHERE userid=?', undef, $count, $id );
...
sub _first_patch_bug_id {
    ...
    Bugzilla->dbh->do( 'UPDATE profiles SET first_patch_bug_id=? WHERE userid=?', undef, $bug_id, $self->id );
