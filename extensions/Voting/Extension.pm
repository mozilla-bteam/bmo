...
sub votes {
    ...
    $self->{votes} = $dbh->selectrow_array( 'SELECT votes FROM bugs WHERE bug_id = ?', undef, $self->id );
    return $self->{votes};
}
...
sub install_update_db {
    ...
    if ( !$field ) {
        Bugzilla::Field->create( { name => 'votes', description => 'Votes', buglist => 1 } );
    }

    $dbh->bz_add_column( 'products', 'votesperuser', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0 } );
    $dbh->bz_add_column( 'products', 'maxvotesperbug',
        { TYPE => 'INT2', NOTNULL => 1, DEFAULT => DEFAULT_VOTES_PER_BUG } );
    $dbh->bz_add_column( 'products', 'votestoconfirm', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0 } );

    $dbh->bz_add_column( 'bugs', 'votes', { TYPE => 'INT3', NOTNULL => 1, DEFAULT => 0 } );
...
sub _bug_user_votes {
    ...
    $self->{'user_votes'} = Bugzilla->dbh->selectrow_array( "SELECT vote_count FROM votes WHERE bug_id = ? AND who = ?",
...
sub template_before_process {
    ...
    if ( $file eq 'admin/users/confirm-delete.html.tmpl' ) {
        ...
        my $votes = Bugzilla->dbh->selectrow_array( 'SELECT COUNT(*) FROM votes WHERE who = ?', undef, $who->id );
        if ($votes) {
            $vars->{other_safe} = 1;
            $vars->{votes}      = $votes;
        }
...
sub bugmail_recipients {
    ...
    my $voters = $dbh->selectcol_arrayref( "SELECT who FROM votes WHERE bug_id = ?", undef, $bug->id );
...
sub sanitycheck_check {
    ...
    my $dbh = Bugzilla->dbh;
    my %cached_counts = @{ $dbh->selectcol_arrayref( 'SELECT bug_id, votes FROM bugs', { Columns => [ 1, 2 ] } ) };

    my %real_counts = @{
        $dbh->selectcol_arrayref( 'SELECT bug_id, SUM(vote_count) FROM votes ' . $dbh->sql_group_by('bug_id'),
            { Columns => [ 1, 2 ] } )
    };
...
sub sanitycheck_repair {
    ...
    my $sth = $dbh->prepare( 'SELECT bug_id, SUM(vote_count) FROM votes ' . $dbh->sql_group_by('bug_id') );
    $sth->execute();
...
sub _check_votes {
    ...
    if ( !defined $votes ) {
        if ( ref $invocant ) {
            ThrowUserError( 'voting_product_illegal_votes', { field => $field, votes => $_[2] } );
        }
        else {
            $votes = $default;
        }
...
sub _update_votes {
    ...
    if (@deleted_votes) {
        $dbh->do( 'DELETE FROM votes WHERE who = ? AND ' . $dbh->sql_in( 'bug_id', \@deleted_votes ), undef, $who );
    }
    ...
    foreach my $bug_id (@updated_bugs) {
        $vars->{'id'} = $bug_id;
        $vars->{'sent_bugmail'} = Bugzilla::BugMail::Send( $bug_id, { 'changer' => $user } );
...
sub _modify_bug_votes {
    ...
    foreach my $bug_id (@updated_bugs) {
        my $sent_bugmail = Bugzilla::BugMail::Send( $bug_id, { changer => Bugzilla->user } );
    ...
    if ( scalar(@list) ) {
        foreach my $ref (@list) {
            ...
            if ($newvotes) {
                ...
                $dbh->do( "DELETE FROM votes WHERE bug_id = ? AND who = ?", undef, ( $id, $userid ) );
            }
        ...
        my $votes = $dbh->selectrow_array( "SELECT SUM(vote_count) " . "FROM votes WHERE bug_id = ?", undef, $id ) || 0;
