...
sub report {
    ...
    if ( $action eq 'run' ) {
        ...
        my $order = ( $input->{'group'} && $input->{'group'} eq 'bug' ) ? 'bug_id, bug_when' : 'bug_when';
        ...
        foreach my $entry (@$list) {
            my ( $fieldname, $bugid, $attachid, $when, $removed, $added, $who, $comment_id ) = @$entry;
            my %change;
            ...
            if ($activity_visible) {
                ...
                if ( $order eq 'bug_when' ) {
                    $is_new_changeset = $operation->{'who'}
                        && ( $who ne $operation->{'who'}
                        || $when ne $operation->{'when'}
                        || $bugid != $operation->{'bug'} );
                }
                else {
                    $is_new_changeset = $operation->{'bug'}
                        && $bugid != $operation->{'bug'};
                }
                if ($is_new_changeset) {
