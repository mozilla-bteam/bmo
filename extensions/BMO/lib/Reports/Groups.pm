...
sub admins_report {
    ...
    my $query = "
        SELECT groups.id, " . $dbh->sql_group_concat( 'profiles.userid', "','", 1 ) . "
          FROM groups
...
sub members_report {
    ...
    my @group_names = sort
        grep { !/^(?:bz_.+|canconfirm|editbugs|editbugs-team|everyone)$/ } @grouplist;
    unshift( @group_names, '' );
                ...
                if ( exists $users{$login} ) {
                    ...
                    if ($privileged) {
                        ...
                        $rh_user->{api_key_only}
                            = $member->settings->{api_key_only}->{value} eq 'on' ? JSON::true : JSON::false;
                    }
                    $users{$login} = $rh_user;
                }
            }
...
sub _filter_userlist {
    ...
    foreach my $user (@$list) {
        ...
        $user->{last_seen_days}
            = sprintf( '%.0f', $now->subtract_datetime_absolute($last_seen)->delta_seconds / ( 28 * 60 * 60 ) );
    }
