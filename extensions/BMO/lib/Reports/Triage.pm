...
use constant DEFAULT_OWNER_PRODUCTS => ( 'Core', 'Firefox', 'Firefox for Android', 'Firefox for iOS', 'Toolkit', );
...
sub unconfirmed {
    ...
    if ( exists $input->{'action'} && $input->{'action'} eq 'run' && $input->{'product'} ) {
        ...
        if ( $input->{'component'} ne '' ) {
            my $ra_components = ref( $input->{'component'} ) ? $input->{'component'} : [ $input->{'component'} ];
