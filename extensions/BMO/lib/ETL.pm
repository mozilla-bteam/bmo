  ...
  my $etl_timer = IO::Async::Timer::Periodic->new(
    ...
    on_tick        => sub {
      try {
        ...
        # Run the export only at the specified hour
        my $datetime = DateTime->now(time_zone => 'local');
        if ($datetime->hour == ETL_RUN_HOUR) {
          INFO('Starting export: ' . $datetime->ymd . ' ' . $datetime->hms);
          my $export = Bugzilla::Extension::BMO::ETL::Export->new(%options);
          $export->run_export;
        }
        else {
          INFO('Skipping ETL export, not the scheduled hour: ' . $datetime->ymd . ' ' . $datetime->hms);
        }
      }
      catch {
        FATAL($_);
