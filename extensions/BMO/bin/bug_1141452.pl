...
foreach my $flag (@$flags) {
    if ( !$flag->{value} ) {
        ...
        $dbh->do( "INSERT INTO bugs_activity(bug_id, who, bug_when, fieldid, removed, added) VALUES (?, ?, ?, ?, ?, ?)",
            undef, $flag->{bug_id}, $nobody->id, $when, $blocking_b2g->id, 'backlog', '---', );
        $dbh->do( "INSERT INTO bugs_activity(bug_id, who, bug_when, fieldid, removed, added) VALUES (?, ?, ?, ?, ?, ?)",
            undef, $flag->{bug_id}, $nobody->id, $when, $tracking_b2g->id, '---', 'backlog', );
    }
    elsif ( $flag->{value} ) {
