...
sub object_end_of_create {
    ...
    if ( $class eq 'Bugzilla::User' ) {
        ...
        foreach my $definition (@default_named_queries) {
            ...
            $dbh->do( "INSERT INTO namedqueries_link_in_footer(namedquery_id,user_id) VALUES (?,?)",
...
sub _bug_reporters_hw_os {
    ...
    if ( !$hw_os ) {
        ( my $ua )
            = Bugzilla->dbh->selectrow_array( "SELECT user_agent FROM bug_user_agent WHERE bug_id = ?", undef,
            $self->id );
        $hw_os
            = $ua
...
sub _product_default_platform {
    ...
    if ( !exists $self->{default_platform} ) {
        $self->{default_platform}
            ...
            ? Bugzilla::Field::Choice->type('rep_platform')->new( $_[0]->{default_platform_id} )->name
            : undef;
    }
...
sub _product_default_op_sys {
    ...
    if ( !exists $self->{default_op_sys} ) {
        $self->{default_op_sys}
            ...
            ? Bugzilla::Field::Choice->type('op_sys')->new( $_[0]->{default_op_sys_id} )->name
            : undef;
    }
...
sub _get_named_query {
    ...
    my ($namedquery_id) = $dbh->selectrow_array( "SELECT id FROM namedqueries WHERE userid=? AND name=?",
    ...
    # and share it
    $dbh->do( "INSERT INTO namedquery_group_map(namedquery_id,group_id) VALUES (?,?)",
...
sub bug_end_of_create {
    ...
    if ( my $ua = Bugzilla->cgi->user_agent ) {
        trick_taint($ua);
        Bugzilla->dbh->do( "INSERT INTO bug_user_agent (bug_id, user_agent) VALUES (?, ?)", undef, $bug->id, $ua );
    }
}
...
sub sanitycheck_check {
    ...
    foreach my $cf_product (@cf_products) {
        $status->( 'bmo_check_cf_visible_in_products_missing', { cf_product => $cf_product }, 'alert' )
...
sub object_start_of_update {
    ...
    return
        unless $new_bug->bug_status eq 'ASSIGNED';
    ...
    return
        unless $old_bug->check_can_change_field( 'bug_status', $old_bug->bug_status, 'NEW' );
...
sub _attachment_external_redirect {
    ...
    return undef
        unless any { $self->contenttype eq $autodetect_attach_urls{$_}->{content_type} }
...
sub _post_gear_bug {
    ...
    my @recipient = (
        "$input->{shiptofirstname} $input->{shiptolastname}", $input->{shiptoemail},
        $input->{shiptoaddress1},                             $input->{shiptoaddress2},
        $input->{shiptocity},                                 $input->{shiptostate},
        $input->{shiptopostcode},                             $input->{shiptocountry},
        "Phone: $input->{shiptophone}",                       $input->{shiptoidrut},
    );
...
sub query_database {
    ...
    if ($query) {
        ...
        if ( $cgi->request_method ne 'POST' ) {
            ThrowCodeError( 'illegal_request_method', { method => $cgi->request_method, accepted => ['POST'] } );
        }
...
sub install_filesystem {
    ...
    my $contribute = eval { $json->decode( scalar read_file( bz_locations()->{cgi_path} . "/contribute.json" ) ); };
...
sub _split_crash_signature {
    ...
    return [ grep {/\S/} extract_multiple( $crash_signature, [ sub { extract_bracketed( $_[0], '[]' ) } ] ) ];
}
