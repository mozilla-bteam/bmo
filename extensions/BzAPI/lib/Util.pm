...
sub fix_bug {
    ...
    my $custom_fields = $stash->{custom_fields} //= Bugzilla->fields( { custom => 1, obsolete => 0, by_name => 1 } );
...
sub fix_comment {
    ...
    if ( $data->{attachment_id} && $method ne 'Bug.search' ) {
        $data->{attachment_ref} = $rpc->type( 'string', ref_urlbase() . "/attachment/" . $object->extra_data );
    }
    else {
