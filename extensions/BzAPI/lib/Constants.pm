...
# Bug: include_fields=_default or empty
sub DEFAULT_BUG_FIELDS {
    ...
    # Add in all custom fields names as well since BzAPI returns them
    # even if not set or visible on this bug
    my $custom_fields = Bugzilla->fields({ custom => 1, obsolete => 0 });
    foreach my $field (@$custom_fields) {
        push(@default_fields, $field->name);
    }

    return @default_fields;
}

# Attachment: include_fields=_default or empty
use constant DEFAULT_ATTACHMENT_FIELDS => (qw(
    attacher
    bug_id
    bug_ref
    content_type
