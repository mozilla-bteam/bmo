...
sub _tracking_flags_search_nonchanged {
    ...
    my ( $bugs_table, $chart_id, $joins, $value, $operator ) = @$args{qw(bugs_table chart_id joins value operator)};
    my $dbh = Bugzilla->dbh;
    ...
    foreach my $flag (@$tracking_flags) {
        ...
        foreach my $value ( @{ $flag->values } ) {
            ...
            if ( !$flag->can_set_value( $value->value ) ) {
                ThrowUserError( 'tracking_flags_change_denied', { flag => $flag, value => $value } );
            }
...
sub bug_check_can_change_field {
    my ( $self, $args ) = @_;
    my ( $bug, $field, $old_value, $new_value, $priv_results ) = @$args{qw(bug field old_value new_value priv_results)};
...
sub bug_end_of_update {
    ...
    foreach my $flag (@$tracking_flags) {
        ...
        if ( $new_value ne $old_value ) {
            ...
            if ( !$flag->can_set_value($new_value) ) {
                ThrowUserError( 'tracking_flags_change_denied', { flag => $flag, value => $new_value } );
            }
            push(
                @flag_changes,
                {
...
sub mailer_before_send {
    ...
    if ( $email->header('X-Bugzilla-ID') ) {
        ...
        my $tracking_flags = Bugzilla::Extension::TrackingFlags::Flag->match( { bug_id => $bug_id } );
        ...
        if (@set_values) {
            ...
            if ( $email->header('X-Bugzilla-Tracking') ) {
                $set_values_string = $email->header('X-Bugzilla-Tracking') . " " . $set_values_string;
            }
...
sub reorg_move_component {
    ...
    Bugzilla->dbh->do( "UPDATE tracking_flags_visibility SET product_id=? WHERE component_id=?",
