        ...
        if ( !@$components ) {
            ...
            foreach my $comp_name (@$components) {
                ...
                foreach my $comp_match (@$comp_matches) {
                    $component_cache{"${prod_name}:${comp_match}"} ||= Bugzilla::Component->new(
                        {
                            name    => $comp_match,
...
sub migrate_flag_values {
    ...
    my %blocking_trusted_requesters = %{$Bugzilla::Extension::BMO::Data::blocking_trusted_requesters};
    my %blocking_trusted_setters    = %{$Bugzilla::Extension::BMO::Data::blocking_trusted_setters};
    my %status_trusted_wanters      = %{$Bugzilla::Extension::BMO::Data::status_trusted_wanters};
    my %status_trusted_setters      = %{$Bugzilla::Extension::BMO::Data::status_trusted_setters};
...
sub migrate_flag_activity {
    ...
    $dbh->do( "UPDATE bugs_activity SET fieldid = ? WHERE fieldid = ?", undef, $new_field->id, $field->id );
...
sub do_migration {
    ...
    foreach my $field (@$fields) {
        ...
        foreach my $field_re ( keys %$bmo_tracking_flags ) {
            ...
            my $new_flag = Bugzilla::Extension::TrackingFlags::Flag->new( { name => $field->name } );
            ...
            # Rename the new flag
            $dbh->do( "UPDATE fielddefs SET name = ? WHERE name = ?", undef, $field->name, $new_flag_name );
