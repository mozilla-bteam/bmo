...
sub page_before_template {
    ...
    my $dbh    = Bugzilla->dbh;
    my @fields = sort
        grep { $_ ne $id_field && $_ ne $order_by; } $dbh->bz_table_columns($table_name);
        ...
        eval {
            ...
            foreach my $row (@$data) {
                ...
                if ( $row->[0] eq '-' ) {
                    ...
                    $dbh->do( "INSERT INTO $table_name(" . join( ',', @fields ) . ") " . "VALUES ($placeholders)",
                        undef, @$row );
                }
                elsif ( $row->[0] < 0 ) {
    ...
    $data = $dbh->selectall_arrayref( "SELECT " . join( ',', @fields ) . " FROM $table_name ORDER BY $order_by" );
