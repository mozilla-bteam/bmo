...
sub _get_activity {
    ...
    $activity_sort_order ||= Bugzilla->user->settings->{'comment_sort_order'}->{'value'};
...
sub bug_end_of_update {
    ...
    foreach my $param ( grep( /^edit_comment_textarea_/, keys %$params ) ) {
        ...
        $dbh->do(
            "INSERT INTO longdescs_activity " . "(comment_id, who, change_when, old_comment) " . "VALUES (?, ?, ?, ?)",
            undef,
