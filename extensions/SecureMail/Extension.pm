...
sub _user_public_key {
    ...
    if ( !exists $self->{public_key} ) {
        ( $self->{public_key} )
            = Bugzilla->dbh->selectrow_array( "SELECT public_key FROM profiles WHERE userid = ?", undef, $self->id );
    }
        ...
        $validators->{'public_key'} = sub {
            ...
            if ( $value =~ /PUBLIC KEY/ ) {
                ...
                if ( !Crypt::OpenPGP::Armour->unarmour($value) ) {
                    ThrowUserError( 'securemail_invalid_key', { errstr => Crypt::OpenPGP::Armour->errstr } );
                }
            }
    ...
    if ( $key_type eq 'PGP' ) {
        ...
        if ( scalar $email->parts > 1 ) {
            ...
            $email->header_set( "Content-Type",
                "multipart/encrypted; " . "protocol=\"application/pgp-encrypted\"; " . "boundary=\"$new_boundary\"" );
        }
        else {
            _fix_encoding($email);
            if ($sanitise_subject) {
        ...
        $template->process( 'account/email/encryption-required.txt.tmpl', $vars, \$message )
            || ThrowTemplateError( $template->error() );
