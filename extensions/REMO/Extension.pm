...
sub _remo_form_payment {
    ...
    if ( $input->{'action'} eq 'commit' ) {
        ...
        if ($token) {
            ...
            if ($old_attach_id) {
                ThrowUserError( 'remo_payment_cancel_dupe', { bugid => $bug_id, attachid => $old_attach_id } );
            }
        }
...
sub post_bug_after_creation {
    ...
    if ( $format eq 'remo-swag' ) {
        ...
        my @new_flags = map { { type_id => $needinfo_type->id, status => '?', requestee => $_->login } } @cc_users;
