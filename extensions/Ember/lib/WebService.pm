...
sub create {
    ...
    $product || ThrowCodeError( 'params_required', { function => 'Ember.create', params => ['product'] } );
...
sub show {
    ...
    $bug_id || ThrowCodeError( 'params_required', { function => 'Ember.show', params => ['id'] } );
...
sub bug {
    ...
    my $bug_id = delete $params->{id};
    $bug_id || ThrowCodeError( 'param_required', { function => 'Ember.bug', param => 'id' } );
    ...
    if ($last_updated) {
        ...
        if ($updated_attachments) {
            $attachments = $self->_get_attachments(
                {
                    attachment_ids => $updated_attachments,
                    exclude_fields => ['data']
                }
            );
        }
...
sub _get_attachments {
    ...
    if ( $params->{ids} ) {
        $attachments = [
            map { @{ $attachments->{bugs}->{$_} } }
                keys %{ $attachments->{bugs} }
        ];
    }
    elsif ( $params->{attachment_ids} ) {
        $attachments = [
            map { $attachments->{attachments}->{$_} }
    ...
    if ($return_groups) {
        push(
            @fields,
            {
                ...
                values => [ map { $self->_group_to_hash( $_, $bug ) } @{ $bug->product_obj->groups_available } ]
            }
        );
    }
