...
if ( grep( /^cmd-/, $cgi->param() ) ) {
    ...
    print $cgi->redirect( "query.cgi?format=" . $cgi->param('query_format') . ( $params ? "&$params" : "" ) );
    exit;
}
...
if ( $action =~ /^(assemble|add|remove|sum|subscribe|unsubscribe)$/ ) {
    ...
    foreach my $cat (qw(category subcategory)) {
        my $is_used
            = $dbh->selectrow_array( "SELECT COUNT(*) FROM series WHERE $cat = ?", undef, $series->{"${cat}_id"} );
        if ( !$is_used ) {
...
sub wrap {
    ...
    $vars->{'imagebase'} = $cgi->canonicalise_query( "action", "action-wrap", "ctype", "format", "width", "height" );
