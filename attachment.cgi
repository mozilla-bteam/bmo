...
sub validateID {
  ...
  return $attachment
    if ($dont_validate_access || check_can_access($attachment));
}
...
sub get_attachment {
  ...
  if (use_attachbase()) {
    ...
    my $cgi_params
      = $cgi->canonicalise_query(@field_names, 't', 'Bugzilla_login',
      'Bugzilla_password');
    ...
    if ($cgi->url_is_attachment_base($bug_id)) {
      ...
      if (!all_attachments_are_public(\%attachments)) {
        ...
        unless ($userid && $valid_token) {
          ...
          # Not a valid token.
          print $cgi->redirect(
            '-location' => Bugzilla->localconfig->{urlbase} . $path);
          exit;
        }
...
sub pack_token_data {
  my $attachments = shift;
  return
    join(' ', map { $_ . '=' . $attachments->{$_}->id } keys %$attachments);
}
...
sub viewall {
  ...
  # Retrieve and validate parameters
  my $bug
    = Bugzilla::Bug->check({id => scalar $cgi->param('bugid'), cache => 1});
...
sub insert {
  ...
  if ($cgi->param('takebug') && $user->in_group('editbugs', $bug->product_id)) {
    ...
    ($bug_status)
      = grep { $_->name eq $bug_status } @{$bug->status->can_change_to};
  ...
  if ($can_edit) {
    ...
    if ($delta_ts && $delta_ts ne $modification_time) {
      ...
      if (scalar(@{$vars->{'operations'}})) {
        ...
        $cgi->param('token',
          issue_hash_token([$attachment->id, $modification_time]));
