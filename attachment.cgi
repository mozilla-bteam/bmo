...
sub validateID {
    ...
    detaint_natural($attach_id)
        || ThrowUserError( "invalid_attach_id", { attach_id => scalar $cgi->param($param) } );
...
sub interdiff {
    ...
    Bugzilla::Attachment::PatchReader::process_interdiff( $old_attachment, $new_attachment, $format, $context );
}
...
sub enter {
    ...
    $vars->{'any_flags_requesteeble'} = grep { $_->is_requestable && $_->is_requesteeble } @$flag_types;
...
sub insert {
    ...
    my ( $flags, $new_flags )
        = Bugzilla::Flag->extract_flags_from_cgi( $bug, $attachment, $vars, SKIP_REQUESTEE_ON_ERROR );
...
sub edit {
    ...
    my $bugattachments = Bugzilla::Attachment->get_attachments_by_bug( $attachment->bug );

    my $any_flags_requesteeble = grep { $_->is_requestable && $_->is_requesteeble } @{ $attachment->flag_types };
...
sub update {
    ...
    $vars->{'bugs'}         = [$bug];
    $vars->{'header_done'}  = 1;
    $vars->{'sent_bugmail'} = Bugzilla::BugMail::Send( $bug->id, { 'changer' => $user } );
