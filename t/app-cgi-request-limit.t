...
my $body     = join("\r\n",
  ...
  '');
my $small_body = join("\r\n",
  "--$boundary", 'Content-Disposition: form-data; name="bug_type"',
  '', 'defect', "--$boundary--", '');
...
)->status_is(413)->content_like(qr{The request is too large\.});
...
$t->post_ok(
  ...
  ->header_like('Access-Control-Allow-Headers' => qr{\bauthorization\b})
  ->header_like('Access-Control-Allow-Headers' => qr{\bx-bugzilla-api-key\b})
  ->header_like('Access-Control-Allow-Headers' => qr{\bx-bugzilla-login\b})
  ->json_is('/error'   => 1)
  ->json_is('/code'    => 58)
  ->json_is('/message' => 'The request is too large.');

$t->post_ok('/rest/component/Test' =>
    {'Content-Length' => 2, 'Content-Type' => 'application/json',} => '{}')
  ->status_is(401)
  ->json_is('/code' => 410);

$t->post_ok('/rest/component/Test' =>
    {'Content-Length' => length($body), 'Content-Type' => 'application/json',} =>
    $body)
  ->status_is(413)
  ->header_like('Content-Type' => qr{^application/json\b})
  ->header_is('Access-Control-Allow-Origin' => '*')
  ->header_like('Access-Control-Allow-Headers' => qr{\bauthorization\b})
  ->header_unlike('Access-Control-Allow-Headers' => qr{\bx-bugzilla-login\b})
  ->json_is('/error'   => 1)
  ->json_is('/code'    => 58)
  ->json_is('/message' => 'The request is too large.');
