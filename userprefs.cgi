...
sub DoAccount {
    ...
    ( $vars->{'realname'} )
        = $dbh->selectrow_array( "SELECT realname FROM profiles WHERE userid = ?", undef, $user->id );
...
sub DoEmail {
    ...
    my $watcher_ids = $dbh->selectcol_arrayref( "SELECT watcher FROM watch WHERE watched = ?", undef, $user->id );
...
sub SaveEmail {
        ...
        || defined $cgi->param('remove_watched_users') )
    {
        ...
        my $old_watch_ids
            = $dbh->selectcol_arrayref( "SELECT watched FROM watch" . " WHERE watcher = ?", undef, $user->id );
        ...
        my $insert_sth = $dbh->prepare( 'INSERT INTO watch (watched, watcher)' . ' VALUES (?, ?)' );
        ...
        if ( defined $cgi->param('remove_watched_users') ) {
            ...
            my $delete_sth = $dbh->prepare( 'DELETE FROM watch WHERE watched = ?' . ' AND watcher = ?' );
    ...
    my ( $removed, $added ) = diff_arrays( [ map { $_->{'id'} } @{ $user->bugs_ignored } ], [ keys %ignored_bugs ] );

    if ( scalar @$removed || scalar @$added ) {
        $dbh->bz_start_transaction();

        if ( scalar @$removed ) {
            $dbh->do( 'DELETE FROM email_bug_ignore WHERE user_id = ? AND ' . $dbh->sql_in( 'bug_id', $removed ),
                undef, $user->id );
        }
        if ( scalar @$added ) {
            my $sth = $dbh->prepare(
...
sub DoPermissions {
    ...
    my $groups = $dbh->selectall_arrayref(
        "SELECT DISTINCT name, description FROM groups WHERE id IN (" . $user->groups_as_string . ") ORDER BY name" );
...
sub DoSessions {
    ...
    my ($count) = $dbh->selectrow_array( "SELECT count(*) FROM logincookies WHERE userid = ?", undef, $user->id );
