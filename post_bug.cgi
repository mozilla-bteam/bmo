...
my $comment;
my $format = $template->get_format( "bug/create/comment", scalar( $cgi->param('format') ), "txt" );
...
my $timestamp = $dbh->selectrow_array( 'SELECT creation_ts FROM bugs WHERE bug_id = ?', undef, $id );
...
$format = $template->get_format( "bug/create/created", scalar( $cgi->param('created-format') ), "html" );
