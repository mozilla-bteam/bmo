...
my @usernames = ( 'admin', 'no-privs', 'QA-Selenium-TEST', 'canconfirm', 'tweakparams', 'permanent_user',
    'editbugs', 'disabled', );
...
my @statuses = (
    {
        value => undef,
        transitions =>
            [ [ 'UNCONFIRMED', 0 ], [ 'CONFIRMED', 0 ], [ 'NEW', 0 ], [ 'ASSIGNED', 0 ], [ 'IN_PROGRESS', 0 ] ],
    },
    {
        value       => 'UNCONFIRMED',
        sortkey     => 100,
        isactive    => 1,
        isopen      => 1,
        transitions => [ [ 'CONFIRMED', 0 ], [ 'NEW', 0 ], [ 'ASSIGNED', 0 ], [ 'IN_PROGRESS', 0 ], [ 'RESOLVED', 0 ] ],
    },
    {
        value    => 'CONFIRMED',
        sortkey  => 200,
        ...
        isopen   => 1,
        transitions =>
            [ [ 'UNCONFIRMED', 0 ], [ 'NEW', 0 ], [ 'ASSIGNED', 0 ], [ 'IN_PROGRESS', 0 ], [ 'RESOLVED', 0 ] ],
    },
    {
        value    => 'NEW',
        sortkey  => 300,
        ...
        isopen   => 1,
        transitions =>
            [ [ 'UNCONFIRMED', 0 ], [ 'CONFIRMED', 0 ], [ 'ASSIGNED', 0 ], [ 'IN_PROGRESS', 0 ], [ 'RESOLVED', 0 ] ],
    },
    {
        value    => 'ASSIGNED',
        sortkey  => 400,
        ...
        isopen   => 1,
        transitions =>
            [ [ 'UNCONFIRMED', 0 ], [ 'CONFIRMED', 0 ], [ 'NEW', 0 ], [ 'IN_PROGRESS', 0 ], [ 'RESOLVED', 0 ] ],
    },
    {
        value       => 'IN_PROGRESS',
        sortkey     => 500,
        isactive    => 1,
        isopen      => 1,
        transitions => [ [ 'UNCONFIRMED', 0 ], [ 'CONFIRMED', 0 ], [ 'NEW', 0 ], [ 'ASSIGNED', 0 ], [ 'RESOLVED', 0 ] ],
    },
    {
        value       => 'REOPENED',
        sortkey     => 600,
    ...
    },
    {
        ...
        isactive    => 1,
        isopen      => 0,
        transitions => [ [ 'UNCONFIRMED', 0 ], [ 'CONFIRMED', 0 ], [ 'REOPENED', 0 ], [ 'VERIFIED', 0 ] ],
    },
    {
        value       => 'VERIFIED',
        sortkey     => 800,
        isactive    => 1,
        isopen      => 0,
        transitions => [ [ 'UNCONFIRMED', 0 ], [ 'CONFIRMED', 0 ], [ 'REOPENED', 0 ], [ 'RESOLVED', 0 ] ],
    },
    {
        value       => 'CLOSED',
        sortkey     => 900,
...
my @products = (
...

    {
        ...
        description      => 'I must remain in the Class2_QA classification ' . 'in all cases! Do not edit!',
...
foreach my $product (@products) {
    ...
    foreach my $milestone ( @{ $product->{milestones} } ) {
            ...
            )
        {
            ...
            $dbh->do( 'INSERT INTO milestones (product_id, value) VALUES (?,?)', undef, $new_product->id, $milestone );
        }
    }
}
...
$dbh->do( 'INSERT INTO group_group_map VALUES (?, ?, 0)', undef, $editbugs->id, $canconfirm->id );
...
foreach my $flag (@flagtypes) {
    ...
    my $request_group_id
        = $flag->{request_group} ? Bugzilla::Group->new( { name => $flag->{request_group} } )->id : undef;
    ...
    $dbh->do(
        'INSERT INTO flagtypes (name, description, cc_list, target_type, is_requestable,
                             ...
                             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        undef,
        (   $flag->{name},             $flag->{desc},           $flag->{cc_list},
            $flag->{target_type},      $flag->{is_requestable}, $flag->{is_requesteeble},
            $flag->{is_multiplicable}, $grant_group_id,         $request_group_id
        )
    );
...
# Don't crash if the entry already exists.
eval { $sth_add_mapping->execute( Bugzilla->user->id, $group->id, 0, GRANT_DIRECT ); };
...
# Don't crash if the entry already exists.
eval { $sth_remove_mapping->execute( Bugzilla->user->id, $group->id, GRANT_DIRECT ); };
