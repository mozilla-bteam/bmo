...
sub get_xmlrpc_client {
    my $config     = get_config();
    my $xmlrpc_url = $config->{browser_url} . "/" . $config->{bugzilla_installation} . "/xmlrpc.cgi";
...
sub file_bug_in_product {
    ...
    if ( $sel->is_text_present($product) ) {
        ...
        $sel->open_ok( "/" . $config->{bugzilla_installation} . "/enter_bug.cgi?product=$product&format=__default__",
