...
sub bz_log_in {
    ...
    my $call = $self->bz_call_success( 'User.login', { login => $username, password => $password } );
...
sub bz_call_fail {
    ...
    if ( defined $faultstring ) {
        cmp_ok( trim( $call->faultstring ), '=~', $faultstring, $self->TYPE . ": Got correct fault for $method" );
    }
    ok( $call->faultcode
            && (
    ...
    foreach my $field ( sort @$fields ) {
        ...
        if ( $field =~ /^is_/ ) {
            ...
            foreach my $cc_item ( @{ $expect->{cc} || [] } ) {
                ok( grep( $_ eq $cc_item, @{ $bug->{cc} } ), $self->TYPE . ": $field contains $cc_item" );
            }
        }
            ...
            is_deeply( $bug->{$field}, $expect->{$field}, $self->TYPE . ": $field value is correct" );
        }
    }
}
