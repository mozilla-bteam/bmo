...
use constant CUSTOM_FIELDS => {
    FIELD_TYPE_FREETEXT,     'cf_freetext',     FIELD_TYPE_SINGLE_SELECT, 'cf_single_select',
    FIELD_TYPE_MULTI_SELECT, 'cf_multi_select', FIELD_TYPE_TEXTAREA,      'cf_textarea',
    FIELD_TYPE_DATETIME,     'cf_datetime',     FIELD_TYPE_BUG_ID,        'cf_bugid',
};
    ...
    anywordssubstr => [
        {
            ...
            value    => '<1> <2>',
            override => {
                ANY_OVERRIDE, percentage_complete => { contains => [ 1, 2, 3 ] },
            }
        },
    ],
    allwordssubstr => [
...
use constant SPECIAL_PARAM_TESTS => (
    ...
    },
    {
        ...
        contains     => [],
        extra_params => {
            email2         => generate_random_password(100),
            emaillongdesc2 => 1,
        },
    },
...
use constant CUSTOM_SEARCH_TESTS => (
...

    {
        ...
        contains => [1],
        params   => [ { f => 'OP' }, { f => 'CP' }, { f => 'bug_id', o => 'equals', v => '<1>' }, ]
    },

    {
        name     => 'Empty OP/CP pair after criteria',
        contains => [1],
        params   => [ { f => 'bug_id', o => 'equals', v => '<1>' }, { f => 'OP' }, { f => 'CP' }, ]
    },
...

    {
        ...
        contains => [1],
        columns  => ['assigned_to'],
        params   => [ { f => 'bug_id', o => 'equals', v => '<1>' }, { f => 'assigned_to', o => 'substr', v => '@' }, ]
    },
...

    {
        ...
        columns    => ['assigned_to'],
        top_params => { j_top => 'OR' },
        params => [ { f => 'bug_id', o => 'equals', v => '<1>' }, { f => 'assigned_to', o => 'equals', v => '<2>' }, ]
    },
    ...
    {
        name       => '(bug_id = 1 AND assigned_to contains @) ' . ' OR (bug_id = 2 AND assigned_to contains @)',
        contains   => [ 1, 2 ],
        columns    => ['assigned_to'],
        top_params => { j_top => 'OR' },
        params     => [
    ...
    {
        name     => '(bug_id = 1 OR assigned_to = 2) ' . ' AND (bug_id = 2 OR assigned_to = 1)',
        contains => [ 1, 2 ],
        columns  => ['assigned_to'],
        params   => [
            { f => 'OP',          j => 'OR' },
    ...
    {
        name     => 'bug_id = 3 OR ( (bug_id = 1 OR assigned_to = 2) ' . ' AND (bug_id = 2 OR assigned_to = 1) )',
        contains => [ 1, 2, 3 ],
        columns  => ['assigned_to'],
        top_params => { j_top => 'OR' },
        params     => [
