...
sub num_tests {
    ...
    my $join_tests = $self->option('long') ? ( $top_combinations * $all_combinations ) : 0;
    ...
    my $sql_injection_tests = scalar(@fields) * scalar(@top_operators) * scalar(@injection_values) * NUM_SEARCH_TESTS;

    # This @{ [] } thing is the only reasonable way to get a count out of a
    # constant array.
    my $special_tests = scalar( @{ [ SPECIAL_PARAM_TESTS, CUSTOM_SEARCH_TESTS ] } ) * TESTS_PER_RUN;
...
sub text_fields {
    my ($self) = @_;
    my @text_fields = grep { $_->type == FIELD_TYPE_TEXTAREA or $_->type == FIELD_TYPE_FREETEXT } $self->all_fields;
...
sub create_legal_value {
    ...
    return $type->create(
        {
            value   => "$number-$field_name-" . random(),
            is_open => 0
        }
    );
}
...
sub _create_field_values {
    ...
    my $classification = Bugzilla::Classification->create( { name => "$number-classification-" . random() } );
    ...
    if ( $number == 1 ) {
        ...
        my @multi_selects = grep { $_->type == FIELD_TYPE_MULTI_SELECT } $self->all_fields;
    ...
    if ( $number == 5 ) {
        my @set_fields = grep { $_->type == FIELD_TYPE_SINGLE_SELECT } $self->all_fields;
...
sub _create_one_bug {
    ...
    my ( $status, $resolution, $see_also ) = delete @params{qw(bug_status resolution see_also)};
    ...
    if ( $number == 5 ) {
        ...
        $dbh->do( 'UPDATE longdescs SET bug_when = ? WHERE bug_id = ?', undef, $ts, $bug->id );
        $bug->{creation_ts}       = $ts;
        ...
        $dbh->do( 'UPDATE longdescs SET bug_when = ? WHERE bug_id = ?', undef, $creation_ts, $bug->id );
        $dbh->do(
...
sub run {
    ...
    foreach my $test (SPECIAL_PARAM_TESTS) {
        my $operator_test = new Bugzilla::Test::Search::OperatorTest( $test->{operator}, $self );
...
sub _setup_dependencies {
    ...
    foreach my $number ( 1, 6 ) {
        ...
        foreach my $row ( [ $original_delta[0], $bug2->id ], [ $original_delta[1], $bug3->id ] ) {
