...
my $sth_run_queries = $dbh->prepare(
    "SELECT " . "query_name, title, onemailperbug " . "FROM whine_queries " . "WHERE eventid=? " . "ORDER BY sortkey" );
my $sth_get_query = $dbh->prepare( "SELECT query FROM namedqueries " . "WHERE userid = ? AND name = ?" );
...
my $sth_schedules_by_event = $dbh->prepare(
    "SELECT id, mailto_type, mailto " . "FROM whine_schedules " . "WHERE eventid=? AND run_next <= NOW()" );
...
my $sched_h = $dbh->prepare( "SELECT id, run_day, run_time " . "FROM whine_schedules " . "WHERE run_next IS NULL" );
$sched_h->execute();
while ( my ( $schedule_id, $day, $time ) = $sched_h->fetchrow_array ) {
    ...
    if ( &check_today($day) ) {
        ...
        # the target time is less than the current time
            ...
            my $run_next = $dbh->sql_date_math( '(' . $dbh->sql_date_math( 'CURRENT_DATE', '+', '?', 'DAY' ) . ')',
                '+', '?', 'HOUR' );
            $sth = $dbh->prepare(
    ...
    # appropriate date and time
        ...
        my $run_next
            = $dbh->sql_date_math( '(' . $dbh->sql_date_math( 'CURRENT_DATE', '+', '?', 'DAY' ) . ')', '+', '?',
            'HOUR' );
        $sth = $dbh->prepare(
            "UPDATE whine_schedules " . "SET run_next = $run_next
                               WHERE id = ?"
...
sub run_queries {
    ...
    foreach my $thisquery (@queries) {
        ...
        if ($@) {
            print STDERR get_text(
                'whine_query_failed',
                {
                    query_name => $thisquery->{'name'},
                    author     => $args->{'author'},
                    reason     => $@
                }
            ) . "\n";
...
sub reset_timer {
    ...
    if ( $minute_offset > 0 ) {
        ...
        my $next_run = $dbh->selectrow_array( 'SELECT ' . $dbh->sql_date_math( 'NOW()', '+', '?', 'MINUTE' ),
            undef, $minute_offset );
