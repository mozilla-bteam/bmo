...
sub ES_SELECT_ALL_SQL {
    ...
    return ( "SELECT $id FROM $table WHERE $id > ? AND is_enabled AND NOT disabledtext ORDER BY $id",
        [ $last_id // 0 ] );
}
...
sub _update_groups {
    ...
    foreach my $is_bless ( keys %$group_changes ) {
        ...
        foreach my $group (@$removed) {
            ...
            Bugzilla->audit(
                sprintf( '%s <%s> removed group %s from %s', $user->login, remote_ip(), $group->name, $self->login ) );
        }
        foreach my $group (@$added) {
            $sth_add_mapping->execute( $self->id, $group->id, $is_bless, GRANT_DIRECT );
            Bugzilla->audit(
                sprintf( '%s <%s> added group %s to %s', $user->login, remote_ip(), $group->name, $self->login ) );
        }
...
sub update_last_seen_date {
    ...
    my $dbh = Bugzilla->dbh;
    my $date = $dbh->selectrow_array( 'SELECT ' . $dbh->sql_date_format( 'NOW()', '%Y-%m-%d' ) );
    ...
    if ( !$self->last_seen_date or $date ne $self->last_seen_date ) {
        ...
        # pending changes
        $dbh->do( "UPDATE profiles SET last_seen_date = ? WHERE userid = ?", undef, $date, $self->id );
...
sub cryptpassword {
    ...
    my ($pw)
        = Bugzilla->dbh->selectrow_array( 'SELECT cryptpassword FROM profiles WHERE userid = ?', undef, $self->id );
    return $pw;
}
...
sub recent_searches {
    my $self = shift;
    $self->{recent_searches} ||= Bugzilla::Search::Recent->match( { user_id => $self->id } );
...
sub force_bug_dissociation {
    ...
    my $user_id     = $self->id;
    my @params      = ( $user_id, $user_id, $user_id, $user_id, map { blessed $_ ? $_->id : $_ } @$groups );
    ...
    foreach my $bug_id (@cc_bugs) {
        ...
        $dbh->do( q{DELETE FROM cc WHERE bug_id = ? AND who = ?}, undef, $bug_id, $self->id );
    }
        ...
        if ( !defined $self->{"product_$product_id"}->{$group} ) {
            ...
            my $in_group = $dbh->selectrow_array(
                "SELECT 1
                             ...
                             WHERE product_id = ?
                                   AND $group != 0
                                   AND " . $self->groups_in_sql . ' ' . $dbh->sql_limit(1),
                undef, $product_id
            );
...
sub visible_bugs {
    ...
    if (@check_ids) {
        ...
        while ( my $row = $sth->fetchrow_arrayref ) {
            my ( $bug_id, $reporter, $owner, $qacontact, $reporter_access, $cclist_access, $isoncclist, $missinggroup )
                = @$row;
...
sub can_enter_product {
    ...
    my $can_enter = $product && grep( $_->name eq $product->name, @{ $self->get_enterable_products } );
    ...
    if ( !$self->in_group('editcomponents') ) {
        ...
        unless ( $e->{0}->{0} ) {
            foreach my $product (@$products) {
                ...
                # its components.
                $can_admin
                    = ( $i->{0}->{0} || $i->{$id}->{0} || scalar( grep { !$e->{$id}->{$_} } keys %{ $i->{$id} } ) );
                last if $can_admin;
            }
        }
...
sub can_set_flag {
    ...
    return ( !$flag_type->grant_group_id || $self->in_group_id( $flag_type->grant_group_id ) )
        ? 1
        : 0;
}
...
sub has_audit_entries {
    ...
    if ( !exists $self->{'has_audit_entries'} ) {
        $self->{'has_audit_entries'}
            = $dbh->selectrow_array( 'SELECT 1 FROM audit_log WHERE user_id = ? ' . $dbh->sql_limit(1), undef,
            $self->id );
    }
...
sub clear_login_failures {
    ...
    my $ip_addr = remote_ip();
    trick_taint($ip_addr);
    Bugzilla->dbh->do( 'DELETE FROM login_failure WHERE user_id = ? AND ip_addr = ?', undef, $self->id, $ip_addr );
    delete $self->{account_ip_login_failures};
}
...
sub account_ip_login_failures {
    my $self    = shift;
    my $dbh     = Bugzilla->dbh;
    my $time    = $dbh->sql_date_math( 'LOCALTIMESTAMP(0)', '-', LOGIN_LOCKOUT_INTERVAL, 'MINUTE' );
    my $ip_addr = remote_ip();
    trick_taint($ip_addr);
    ...
    my $eventdata = $dbh->selectrow_array(
        "SELECT eventdata
          ...
          WHERE (tokentype = 'emailold'
                AND SUBSTRING(eventdata, 1, (" . $dbh->sql_position( q{':'}, 'eventdata' ) . "-  1)) = ?)
