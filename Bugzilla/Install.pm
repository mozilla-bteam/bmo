...
sub update_settings {
    ...
    my $any_settings = $dbh->selectrow_array( 'SELECT 1 FROM setting ' . $dbh->sql_limit(1) );
    if ( !$any_settings ) {
...
sub init_workflow {
    ...
    my %status_ids = @{ $dbh->selectcol_arrayref( 'SELECT value, id FROM bug_status', { Columns => [ 1, 2 ] } ) };
...
sub create_admin {
    ...
    my $admin_group      = new Bugzilla::Group( { name => 'admin' } );
    my $admin_inheritors = Bugzilla::Group->flatten_group_membership( $admin_group->id );
