...
sub _sqlite_create_table {
    my ( $self, $table ) = @_;
    return scalar Bugzilla->dbh->selectrow_array( "SELECT sql FROM sqlite_master WHERE name = ? AND type = 'table'",
        undef, $table );
}
...
sub get_add_column_ddl {
    ...
    if ( $definition->{NOTNULL} and !defined $definition->{DEFAULT} ) {
        ...
        my $table_sql = $self->_sqlite_new_column_sql(@_);
        return $self->_sqlite_alter_schema( $table, $table_sql, { pre_sql => \@pre_sql, extra_column => $column } );
    }
