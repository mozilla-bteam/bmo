...
sub new {
    ...
    my $attrs = {
        ...
        LongReadLen      => max( Bugzilla->params->{'maxattachmentsize'} || 0, MIN_LONG_READ_LEN ) * 1024,
    };
...
sub bz_check_regexp {
    ...
    eval { $self->do( "SELECT 1 FROM DUAL WHERE " . $self->sql_regexp( $self->quote("a"), $pattern, 1 ) ) };
...
sub adjust_statement {
    ...
    if ( defined($limit) ) {
        ...
        if ( defined($offset) ) {
            ...
            $before_where
                = "$before_from FROM ($before_from," . " ROW_NUMBER() OVER (ORDER BY 1) R " . " FROM $after_from ) ";
...
sub _get_create_seq_ddl {
    ...
    my $seq_sql     = "CREATE SEQUENCE $seq_name INCREMENT BY 1 START WITH 1 " . "NOMAXVALUE NOCYCLE NOCACHE";
