...
use constant ABSTRACT_SCHEMA => {
    ...
    longdescs_tags => {
        ...
        INDEXES => [ longdescs_tags_idx => { FIELDS => [ 'comment_id', 'tag' ], TYPE => 'UNIQUE' }, ],
    },
    ...
    longdescs_tags_weights => {
        ...
        INDEXES => [ longdescs_tags_weights_tag_idx => { FIELDS => ['tag'], TYPE => 'UNIQUE' }, ],
    },
    ...
    profile_mfa => {
        ...
        INDEXES => [ profile_mfa_userid_name_idx => { FIELDS => [ 'user_id', 'name' ], TYPE => 'UNIQUE' }, ],
    },
    ...
    namedqueries_link_in_footer => {
        ...
        INDEXES => [
            namedqueries_link_in_footer_id_idx => {
                FIELDS => [qw(namedquery_id user_id)],
                TYPE   => 'UNIQUE'
            },
    ...
    tag => {
        ...
        INDEXES => [ tag_user_id_idx => { FIELDS => [qw(user_id name)], TYPE => 'UNIQUE' }, ],
    },
    ...
    bug_tag => {
        ...
        INDEXES => [ bug_tag_bug_id_idx => { FIELDS => [qw(bug_id tag_id)], TYPE => 'UNIQUE' }, ],
    },
    ...
    component_cc => {
        ...
        INDEXES => [
            component_cc_user_id_idx => {
                FIELDS => [qw(component_id user_id)],
                TYPE   => 'UNIQUE'
            },
        ],
    },
    ...
    group_control_map => {
        ...
        INDEXES => [
            group_control_map_product_id_idx => { FIELDS => [qw(product_id group_id)], TYPE => 'UNIQUE' },
    ...
    setting_value => {
        ...
        INDEXES => [
            setting_value_nv_unique_idx => {
                FIELDS => [qw(name value)],
                TYPE   => 'UNIQUE'
            },
            setting_value_ns_unique_idx => {
                FIELDS => [qw(name sortindex)],
                TYPE   => 'UNIQUE'
            },
        ],
    },
    ...
    profile_setting => {
        ...
        INDEXES => [
            profile_setting_value_unique_idx => {
                FIELDS => [qw(user_id setting_name)],
                TYPE   => 'UNIQUE'
            },
        ],
    },
...
=cut
    ...
    if ($driver) {
        ...
        $class .= '::' . $subclass;
        eval "require $class;";
        die "The $class class could not be found ($subclass " . "not supported?): $@" if ($@);
    }
...
=cut
    ...
    $sql .= "INDEX $index_name ON $table_name \(" . join( ", ", @$index_fields ) . "\)";
...
=cut
    ...
    my @statements;
    push( @statements, "ALTER TABLE $table " . $self->ADD_COLUMN . " $column " . $self->get_type_ddl($definition) );
    ...
    if ( defined $definition->{REFERENCES} ) {
        push( @statements, $self->get_add_fks_sql( $table, { $column => $definition->{REFERENCES} } ) );
    }
...
=cut
    ...
    {
        push( @statements, "ALTER TABLE $table ALTER COLUMN $column " . " SET DEFAULT $default" );
    }
