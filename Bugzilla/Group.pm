...
sub bugs {
    ...
    my $bug_ids
        = Bugzilla->dbh->selectcol_arrayref( 'SELECT bug_id FROM bug_group_map WHERE group_id = ?', undef, $self->id );
...
sub members_complete {
    ...
    my $sth = $dbh->prepare( "SELECT DISTINCT user_id FROM user_group_map WHERE isbless = 0 AND group_id = ?" );
