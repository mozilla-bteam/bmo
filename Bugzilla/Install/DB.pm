...
sub update_fielddefs_definition {
    ...
    if ( !$dbh->bz_column_info( 'fielddefs', 'obsolete' ) ) {
        $dbh->bz_add_column( 'fielddefs', 'obsolete', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    ...
    # in fielddefs.
    $dbh->bz_add_column( 'fielddefs', 'type',   { TYPE => 'INT2',    NOTNULL => 1, DEFAULT => 0 } );
    $dbh->bz_add_column( 'fielddefs', 'custom', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );

    $dbh->bz_add_column( 'fielddefs', 'enter_bug', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    ...
    if ( !$dbh->bz_column_info( 'fielddefs', 'buglist' ) ) {
        $dbh->bz_add_column( 'fielddefs', 'buglist', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    ...
    $dbh->bz_add_column( 'fielddefs', 'is_numeric', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    $dbh->do( 'UPDATE fielddefs SET is_numeric = 1 WHERE type = ' . FIELD_TYPE_BUG_ID );
...
sub update_table_definitions {
        ...
        || !$dbh->bz_index_info( 'keywords', 'keywords_bug_id_idx' )->{TYPE} )
    {
        ...
        $dbh->bz_add_index( 'keywords', 'keywords_bug_id_idx', { TYPE => 'UNIQUE', FIELDS => [qw(bug_id keywordid)] } );
    }
    ...
    $dbh->bz_add_column( 'groups', 'isactive', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'TRUE' } );

    $dbh->bz_add_column( 'attachments', 'isobsolete', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    ...
    if ( !$dbh->bz_column_info( 'profiles', 'disabledtext' )->{NOTNULL} ) {
        $dbh->bz_alter_column( "profiles", "disabledtext", { TYPE => 'MEDIUMTEXT', NOTNULL => 1 }, '' );
    }

    $dbh->bz_add_column( "bugs", "reporter_accessible", { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'TRUE' } );
    $dbh->bz_add_column( "bugs", "cclist_accessible",   { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'TRUE' } );
    ...
    $dbh->bz_add_column( "longdescs", "work_time",      { TYPE => 'decimal(5,2)', NOTNULL => 1, DEFAULT => '0' } );
    $dbh->bz_add_column( "bugs",      "estimated_time", { TYPE => 'decimal(5,2)', NOTNULL => 1, DEFAULT => '0' } );
    $dbh->bz_add_column( "bugs",      "remaining_time", { TYPE => 'decimal(5,2)', NOTNULL => 1, DEFAULT => '0' } );
    $dbh->bz_add_column( "bugs",      "deadline",       { TYPE => 'DATETIME' } );

    _use_ip_instead_of_hostname_in_logincookies();

    $dbh->bz_add_column( 'longdescs',   'isprivate', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    $dbh->bz_add_column( 'attachments', 'isprivate', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );

    $dbh->bz_add_column( "bugs", "alias", { TYPE => "varchar(20)" } );
    $dbh->bz_add_index( 'bugs', 'bugs_alias_idx', { TYPE => 'UNIQUE', FIELDS => [qw(alias)] } );
    ...
    $dbh->bz_add_column( 'whine_schedules', 'mailto_type', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => '0' } );
    ...
    $dbh->bz_alter_column( 'bugs', 'bug_status', { TYPE => 'varchar(64)', NOTNULL => 1 } );

    # 2005-03-23 Tomas.Kopal@altap.cz - add default value to resolution,
    # bug 286695
    $dbh->bz_alter_column( 'bugs', 'resolution', { TYPE => 'varchar(64)', NOTNULL => 1, DEFAULT => "''" } );
    $dbh->bz_alter_column( 'bugs', 'priority', { TYPE => 'varchar(64)', NOTNULL => 1 } );
    $dbh->bz_alter_column( 'bugs', 'bug_severity', { TYPE => 'varchar(64)', NOTNULL => 1 } );
    $dbh->bz_alter_column( 'bugs', 'rep_platform', { TYPE => 'varchar(64)', NOTNULL => 1 }, '' );
    ...
    $dbh->bz_add_column( 'quips', 'approved', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'TRUE' } );
    ...
    $dbh->bz_add_column( 'products', 'classification_id', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => '1' } );
    ...
    $dbh->bz_alter_column( 'bugs', 'status_whiteboard', { TYPE => 'MEDIUMTEXT', NOTNULL => 1, DEFAULT => "''" } );
    if ( $dbh->bz_column_info( 'bugs', 'votes' ) ) {
        $dbh->bz_alter_column( 'bugs', 'votes', { TYPE => 'INT3', NOTNULL => 1, DEFAULT => '0' } );
    }
    ...
    $dbh->bz_alter_column( 'products', 'classification_id', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => '1' } );

    # initialowner was accidentally NULL when we checked-in Schema,
    # when it really should be NOT NULL.
    $dbh->bz_alter_column( 'components', 'initialowner', { TYPE => 'INT3', NOTNULL => 1 }, 0 );
    ...
    $dbh->bz_alter_column( 'versions', 'value', { TYPE => 'varchar(64)', NOTNULL => 1 } );
    _add_versions_product_id_index();

    if ( !exists $dbh->bz_column_info( 'milestones', 'sortkey' )->{DEFAULT} ) {
        $dbh->bz_alter_column( 'milestones', 'sortkey', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0 } );
    }
    ...
    $dbh->bz_alter_column( 'groups', 'userregexp', { TYPE => 'TINYTEXT', NOTNULL => 1, DEFAULT => "''" } );

    _clean_control_characters_from_short_desc();

    # 2005-12-07 altlst@sonic.net -- Bug 225221
    $dbh->bz_add_column( 'longdescs', 'comment_id', { TYPE => 'INTSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );

    _stop_storing_inactive_flags();
    _change_short_desc_from_mediumtext_to_varchar();

    # 2006-07-01 wurblzap@gmail.com -- Bug 69000
    $dbh->bz_add_column( 'namedqueries', 'id', { TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
    ...
    if ( $dbh->bz_column_info( 'products', 'milestoneurl' ) ) {
        $dbh->bz_alter_column( 'products', 'milestoneurl', { TYPE => 'TINYTEXT', NOTNULL => 1, DEFAULT => "''" } );
    }
    if ( $dbh->bz_column_info( 'products', 'disallownew' ) ) {
        $dbh->bz_alter_column( 'products', 'disallownew', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 0 } );

        if ( $dbh->bz_column_info( 'products', 'votesperuser' ) ) {
            $dbh->bz_alter_column( 'products', 'votesperuser',   { TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0 } );
            $dbh->bz_alter_column( 'products', 'votestoconfirm', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0 } );
        }
    }
    ...
    $dbh->bz_alter_column( 'profiles', 'disabledtext', { TYPE => 'MEDIUMTEXT',   NOTNULL => 1, DEFAULT => "''" } );
    $dbh->bz_alter_column( 'profiles', 'realname',     { TYPE => 'varchar(255)', NOTNULL => 1, DEFAULT => "''" } );

    _update_longdescs_who_index();

    $dbh->bz_add_column( 'setting', 'subclass', { TYPE => 'varchar(32)' } );

    $dbh->bz_alter_column( 'longdescs', 'thetext', { TYPE => 'LONGTEXT', NOTNULL => 1 }, '' );

    # 2006-10-20 LpSolit@gmail.com - Bug 189627
    $dbh->bz_add_column( 'group_control_map', 'editcomponents',
        { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    $dbh->bz_add_column( 'group_control_map', 'editbugs',   { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    $dbh->bz_add_column( 'group_control_map', 'canconfirm', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );

    # 2006-11-07 LpSolit@gmail.com - Bug 353656
    $dbh->bz_add_column( 'longdescs', 'type', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => '0' } );
    $dbh->bz_add_column( 'longdescs', 'extra_data', { TYPE => 'varchar(255)' } );

    $dbh->bz_add_column( 'versions',   'id', { TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
    $dbh->bz_add_column( 'milestones', 'id', { TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
    ...
    $dbh->bz_alter_column( 'group_control_map', 'product_id', { TYPE => 'INT2', NOTNULL => 1 } );
    ...
    $dbh->bz_alter_column( 'group_control_map', 'entry',   { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    $dbh->bz_alter_column( 'group_control_map', 'canedit', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );

    # 2009-01-16 oreomike@gmail.com - Bug 302420
    $dbh->bz_add_column( 'whine_events', 'mailifnobugs', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    ...
    # attachments.ispatch.
    $dbh->bz_alter_column( 'attachments', 'ispatch', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    $dbh->bz_alter_column( 'keyworddefs', 'description', { TYPE => 'MEDIUMTEXT', NOTNULL => 1 }, '' );
    $dbh->bz_alter_column( 'products',    'description', { TYPE => 'MEDIUMTEXT', NOTNULL => 1 }, '' );

    # Change the default of allows_unconfirmed to TRUE as part
    # of the new workflow.
    $dbh->bz_alter_column( 'products', 'allows_unconfirmed', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'TRUE' } );
    ...
    $dbh->bz_add_column( 'bug_see_also', 'id', { TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
    ...
    $dbh->bz_add_column( 'profiles_activity', 'id', { TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
    ...
    $dbh->bz_add_index( 'bug_user_last_visit', 'bug_user_last_visit_last_visit_ts_idx', ['last_visit_ts'] );

    # 2014-07-14 sgreen@redhat.com - Bug 726696
    $dbh->bz_alter_column( 'tokens', 'tokentype', { TYPE => 'varchar(16)', NOTNULL => 1 } );
    ...
    $dbh->bz_add_column( 'keyworddefs', 'is_active', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'TRUE' } );

    $dbh->bz_add_column( 'user_api_keys', 'app_id', { TYPE => 'varchar(64)' } );
    $dbh->bz_add_index( 'user_api_keys', 'user_api_keys_user_id_app_id_idx', [qw(user_id app_id)] );
    ...
    $dbh->bz_alter_column( 'tokens', 'token', { TYPE => 'varchar(22)', NOTNULL => 1, PRIMARYKEY => 1 } );

    # 2015-08-20 dylan@mozilla.com - Bug 1196092
    $dbh->bz_alter_column( 'logincookies', 'cookie', { TYPE => 'varchar(22)', NOTNULL => 1 } );
    $dbh->bz_add_index( 'logincookies', 'logincookies_cookie_idx', { TYPE => 'UNIQUE', FIELDS => ['cookie'] } );
    $dbh->bz_add_column( 'logincookies', 'id', { TYPE => 'INTSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
    ...
    $dbh->bz_add_column( 'groups', 'idle_member_removal', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => '0' } );
...
sub _update_pre_checksetup_bugzillas {
    ...
    $dbh->bz_add_column( 'bugs', 'target_milestone',  { TYPE => 'varchar(20)', NOTNULL => 1, DEFAULT => "'---'" } );
...
sub _add_bug_vote_cache {
    ...
    if ( $dbh->bz_column_info( 'bugs', 'area' ) ) {
        ...
        $dbh->bz_add_column( 'products', 'votesperuser', { TYPE => 'INT2', NOTNULL => 1 }, 0 );
    }
}
...
sub _update_product_name_definition {
    ...
    if ( $dbh->bz_column_info( 'products', 'product' ) ) {
        ...
        $dbh->bz_alter_column( 'versions',   'program', { TYPE => 'varchar(64)', NOTNULL => 1 } );
    }
}
...
sub _update_bugs_activity_field_to_fieldid {
    ...
    if ( $dbh->bz_column_info( 'bugs_activity', 'field' ) ) {
        $dbh->bz_add_column( 'bugs_activity', 'fieldid', { TYPE => 'INT3', NOTNULL => 1 }, 0 );
        ...
        foreach my $item (@$ids) {
            ...
            # If the id is NULL
            if ( !$id ) {
                $dbh->do( "INSERT INTO fielddefs (name, description) VALUES " . "(?, ?)", undef, $field, $field );
                $id = $dbh->bz_last_key( 'fielddefs', 'id' );
            }
            $dbh->do( "UPDATE bugs_activity SET fieldid = ? WHERE field = ?", undef, $id, $field );
        }
        $dbh->bz_commit_transaction();
...
sub _add_unique_login_name_index_to_profiles {
        ...
        || !$dbh->bz_index_info( 'profiles', 'profiles_login_name_idx' )->{TYPE} )
    {
        ...
        $dbh->bz_add_index( 'profiles', 'profiles_login_name_idx', { TYPE => 'UNIQUE', FIELDS => [qw(login_name)] } );
    }
}
...
sub _update_component_user_fields_to_ids {
    ...
    if ( $comp_init_owner && $comp_init_owner->{TYPE} eq 'TINYTEXT' ) {
        ...
        while ( my ( $program, $value, $initialowner ) = $sth->fetchrow_array() ) {
            my ($id)
                = $dbh->selectrow_array( "SELECT userid FROM profiles WHERE login_name = ?", undef, $initialowner );
    ...
    if ( $comp_init_qa && $comp_init_qa->{TYPE} eq 'TINYTEXT' ) {
        ...
        while ( my ( $program, $value, $initialqacontact ) = $sth->fetchrow_array() ) {
            my ($id)
                = $dbh->selectrow_array( "SELECT userid FROM profiles WHERE login_name = ?", undef, $initialqacontact );
...
sub _add_products_defaultmilestone {
    ...
    if ( !$dbh->bz_column_info( 'products', 'defaultmilestone' ) ) {
        ...
        while ( my ( $product, $default_ms ) = $sth->fetchrow_array() ) {
            ...
            if ( !$exists ) {
                $dbh->do( "INSERT INTO milestones(value, product) " . "VALUES (?, ?)", undef, $default_ms, $product );
            }
        }
    }
}
...
sub _use_ids_for_products_and_components {
    ...
    if ( $dbh->bz_column_info( "products", "product" ) ) {
        ...
        $dbh->bz_add_column( "products", "id", { TYPE => 'SMALLSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
        $dbh->bz_add_column( "components", "product_id", { TYPE => 'INT2', NOTNULL => 1 }, 0 );
        $dbh->bz_add_column( "versions",   "product_id", { TYPE => 'INT2', NOTNULL => 1 }, 0 );
        $dbh->bz_add_column( "milestones", "product_id", { TYPE => 'INT2', NOTNULL => 1 }, 0 );
        ...
        if ( $dbh->bz_table_info("attachstatusdefs") ) {
            $dbh->bz_add_column( "attachstatusdefs", "product_id", { TYPE => 'INT2', NOTNULL => 1 }, 0 );
        }
        ...
        while ( my ( $product_id, $product ) = $sth->fetchrow_array() ) {
            ...
            $products{$product} = 1;
            $dbh->do( "UPDATE components SET product_id = $product_id " . "WHERE program = " . $dbh->quote($product) );
            $dbh->do( "UPDATE versions SET product_id = $product_id " . "WHERE program = " . $dbh->quote($product) );
            $dbh->do( "UPDATE milestones SET product_id = $product_id " . "WHERE product = " . $dbh->quote($product) );
            $dbh->do( "UPDATE bugs SET product_id = $product_id " . "WHERE product = " . $dbh->quote($product) );
            $dbh->do(
                "UPDATE attachstatusdefs SET product_id = $product_id " . "WHERE product = " . $dbh->quote($product) )
                if $dbh->bz_table_info("attachstatusdefs");
        }

        print "Updating the database to use component IDs.\n";

        $dbh->bz_add_column( "components", "id", { TYPE => 'SMALLSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
        ...
        $dbh->bz_alter_column( "products", "name", { TYPE => 'varchar(64)', NOTNULL => 1 } );
        $dbh->bz_rename_column( "components", "value", "name" );
        $dbh->bz_alter_column( "components", "name", { TYPE => 'varchar(64)', NOTNULL => 1 } );

        print "Adding indexes for products and components tables.\n";
        $dbh->bz_add_index( 'products', 'products_name_idx', { TYPE => 'UNIQUE', FIELDS => [qw(name)] } );
...
sub _convert_groups_system_from_groupset {
    ...
    if ( $dbh->bz_column_info( "profiles", "groupset" ) ) {
        ...
        $dbh->bz_add_column( 'groups', 'id', { TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );

        $dbh->bz_add_index( 'groups', 'groups_name_idx', { TYPE => 'UNIQUE', FIELDS => [qw(name)] } );
        ...
        my ($admin_gid) = $dbh->selectrow_array("SELECT id FROM groups WHERE name = 'admin'");
        if ( !$admin_gid ) {
            $dbh->do(
...
sub _convert_attachment_statuses_to_flags {
        ...
        && $dbh->bz_table_info("attachstatusdefs") )
    {
        ...
        my ($old_field_id) = $dbh->selectrow_array("SELECT id FROM fielddefs WHERE name='attachstatusdefs.name'")
            || 0;
        my ($new_field_id) = $dbh->selectrow_array("SELECT id FROM fielddefs WHERE name = 'flagtypes.name'");
        ...
        $sth->execute();
        while ( my ( $attach_id, $who, $when, $old_added, $old_removed ) = $sth->fetchrow_array() ) {
...
sub _setup_usebuggroups_backward_compatibility {
    ...
    my ($maps_exist) = $dbh->selectrow_array("SELECT DISTINCT 1 FROM group_control_map");
    if ( !$maps_exist ) {
        ...
        $sth->execute();
        while ( my ( $groupid, $productid, $groupname, $productname ) = $sth->fetchrow_array() ) {
...
sub _remove_user_series_map {
    ...
    if ( $dbh->bz_table_info("user_series_map") ) {
        ...
        $dbh->bz_add_column( "series", "public", { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );

        # Migrate public-ness across from user_series_map to new field
        my $sth = $dbh->prepare( "SELECT series_id from user_series_map " . "WHERE user_id = 0" );
        $sth->execute();
        while ( my ($public_series_id) = $sth->fetchrow_array() ) {
            $dbh->do( "UPDATE series SET public = 1 " . "WHERE series_id = $public_series_id" );
        }
...
sub _copy_old_charts_into_database {
    ...
    my $series_exists = $dbh->selectrow_array( "SELECT 1 FROM series " . $dbh->sql_limit(1) );
    if ( !$series_exists && -d "$datadir/mining" && -e "$datadir/mining/-All-" ) {
        ...
        my $deletesth = $dbh->prepare("DELETE FROM series_data WHERE series_id = ? AND series_date = ?");
        ...
        foreach my $product ( ( map { $_->[0] } @$products ), "-All-" ) {
            ...
            foreach my $field (@fields) {
                ...
                my $series = new Bugzilla::Series( undef, $product, $all_name, $field, undef, 1, $queries{$field}, 1 );
            ...
            my $series
                = new Bugzilla::Series( undef, $product, $all_name, $open_name, undef, 1, $query_prod . $query, 1 );
            $series->writeToDatabase();
            ...
            my $product_id
                = $dbh->selectrow_array( "SELECT id FROM products " . "WHERE name = " . $dbh->quote($product) );
...
sub _add_user_group_map_grant_type {
    ...
    if ( $dbh->bz_column_info( "user_group_map", "isderived" ) ) {
        $dbh->bz_add_column( 'user_group_map', 'grant_type', { TYPE => 'INT1', NOTNULL => 1, DEFAULT => '0' } );
...
sub _add_group_group_map_grant_type {
    ...
    if ( $dbh->bz_column_info( "group_group_map", "isbless" ) ) {
        $dbh->bz_add_column( 'group_group_map', 'grant_type', { TYPE => 'INT1', NOTNULL => 1, DEFAULT => '0' } );
        $dbh->do(
            "UPDATE group_group_map SET grant_type = " . "IF(isbless, " . GROUP_BLESS . ", " . GROUP_MEMBERSHIP . ")" );
...
sub _convert_attachments_filename_from_mediumtext {
    ...
    if ( $ref->{TYPE} ne 'varchar(100)' ) {
        ...
        while ( my ( $attach_id, $filename ) = $sth->fetchrow_array ) {
            ...
            $dbh->do( "UPDATE attachments SET filename = $quoted_filename " . "WHERE attach_id = $attach_id" );
        }

        print "Done.\n";

        $dbh->bz_alter_column( "attachments", "filename", { TYPE => 'varchar(100)', NOTNULL => 1 } );
    }
}
...
sub _fix_group_with_empty_name {
    ...
    my ($emptygroupid) = $dbh->selectrow_array("SELECT id FROM groups where name = ''");
    if ($emptygroupid) {
        ...
        $dbh->do( "UPDATE groups SET name = ? WHERE id = ?", undef, $trygroupname, $emptygroupid );
...
sub _migrate_email_prefs_to_new_table {
    ...
    if ( $dbh->bz_column_info( "profiles", "emailflags" ) ) {
        ...
        while ( my ( $userid, $flagstring ) = $sth->fetchrow_array() ) {
            ...
            my $sth2 = $dbh->prepare(
                "INSERT into email_setting " . "(user_id, relationship, event) VALUES (" . "$userid, ?, ?)" );
...
sub _initialize_new_email_prefs {
    ...
    foreach my $desc ( keys %events ) {
        ...
        my $have_events
            = $dbh->selectrow_array( "SELECT 1 FROM email_setting WHERE event = $event " . $dbh->sql_limit(1) );
    ...
    if ( $dbh->isa('Bugzilla::DB::Mysql') ) {
            ...
            and $approved_col->{COLUMN_SIZE} == 1 )
        {
            ...
            if ( $dbh->bz_column_info( 'series', 'public' ) ) {
                $dbh->bz_alter_column_raw( 'series', 'public', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => '0' } );
            }
            $dbh->bz_alter_column_raw( 'bug_status', 'isactive', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => '1' } );
            $dbh->bz_alter_column_raw( 'rep_platform', 'isactive',
                { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => '1' } );
            $dbh->bz_alter_column_raw( 'resolution', 'isactive', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => '1' } );
            $dbh->bz_alter_column_raw( 'op_sys',     'isactive', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => '1' } );
            $dbh->bz_alter_column_raw( 'bug_severity', 'isactive',
                { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => '1' } );
            $dbh->bz_alter_column_raw( 'priority', 'isactive', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => '1' } );
            $dbh->bz_alter_column_raw( 'quips',    'approved', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => '1' } );
        }
    }
}
...
sub _de_dup_version {
    ...
    $dbh->do( 'DELETE FROM versions WHERE product_id = ? AND value = ?', undef, $product_id, $version );
    $dbh->do( 'INSERT INTO versions (product_id, value) VALUES (?,?)',   undef, $product_id, $version );
}
...
sub _fix_whine_queries_title_and_op_sys_value {
    ...
    if ( !exists $dbh->bz_column_info( 'whine_queries', 'title' )->{DEFAULT} ) {
        ...
        $dbh->do( 'UPDATE op_sys SET value = ? WHERE value = ?', undef, "Other", "other" );
        $dbh->do( 'UPDATE bugs SET op_sys = ? WHERE op_sys = ?', undef, "Other", "other" );
...
sub _fix_attachments_submitter_id_idx {
    ...
    if ( $dbh->bz_index_info( 'attachments', 'attachments_submitter_id_idx' )
        && ( scalar( @{ $dbh->bz_index_info( 'attachments', 'attachments_submitter_id_idx' )->{FIELDS} } ) < 2 ) )
    {
        $dbh->bz_drop_index( 'attachments', 'attachments_submitter_id_idx' );
    }
    $dbh->bz_add_index( 'attachments', 'attachments_submitter_id_idx', [qw(submitter_id bug_id)] );
}
...
sub _fix_broken_all_closed_series {
    ...
    my $broken_series_indicator = 'field0-0-0=resolution&type0-0-0=notequals&value0-0-0=---';
    ...
    if (@$broken_nonopen_series) {
        ...
        my $sth_repair = $dbh->prepare('UPDATE series SET query = ? WHERE series_id = ?');
        ...
        my $open_bugs_query_base_old
            = join( "&", map { "bug_status=" . url_quote($_) } ( 'UNCONFIRMED', 'NEW', 'ASSIGNED', 'REOPENED' ) );
        my $open_bugs_query_base_new
            = join( "&", map { "bug_status=" . url_quote($_) } ( 'NEW', 'REOPENED', 'ASSIGNED', 'UNCONFIRMED' ) );
        my $sth_openbugs_series = $dbh->prepare("SELECT series_id FROM series WHERE query IN (?, ?)");
        ...
        my $sth_select_broken_nonopen_data
            = $dbh->prepare( 'SELECT series_date, series_value FROM series_data' . ' WHERE series_id = ?' );

        # Statement to select an open bugs count data entry.
        my $sth_select_open_data
            = $dbh->prepare( 'SELECT series_value FROM series_data' . ' WHERE series_id = ? AND series_date = ?' );

        # Statement to fix a broken non-open bugs count data entry.
        my $sth_fix_broken_nonopen_data
            = $dbh->prepare( 'UPDATE series_data SET series_value = ?' . ' WHERE series_id = ? AND series_date = ?' );

        # Statement to delete an unfixable broken non-open bugs count data
        # entry.
        my $sth_delete_broken_nonopen_data
            = $dbh->prepare( 'DELETE FROM series_data' . ' WHERE series_id = ? AND series_date = ?' );
            ...
            if ( $nonopen_bugs_query =~ /^$broken_series_indicator(.*)$/ ) {
                ...
                if ($found_open_series_id) {
                    ...
                    while ( my $rowref = $sth_select_broken_nonopen_data->fetchrow_arrayref ) {
                        ...
                        my ($openbugs_value)
                            = $dbh->selectrow_array( $sth_select_open_data, undef, $found_open_series_id, $date );
...
sub _rederive_regex_groups {
    ...
    my $regex_groups_exist
        = $dbh->selectrow_array( "SELECT 1 FROM groups WHERE userregexp = '' " . $dbh->sql_limit(1) );
    return if !$regex_groups_exist;

    my $regex_derivations = $dbh->selectrow_array(
        'SELECT 1 FROM user_group_map WHERE grant_type = ' . GRANT_REGEXP . ' ' . $dbh->sql_limit(1) );
...
sub _clean_control_characters_from_short_desc {
    ...
    my $controlchar_bugs = $dbh->selectall_arrayref(
        "SELECT short_desc, bug_id FROM bugs WHERE " . $dbh->sql_regexp( 'short_desc', "'[[:cntrl:]]'" ) );
    if ( scalar(@$controlchar_bugs) ) {
        ...
        foreach (@$controlchar_bugs) {
            ...
            if ( $clean_short_desc ne $short_desc ) {
                ...
                $found = 1;
                print " $bug_id...";
                $dbh->do( "UPDATE bugs SET short_desc = ? WHERE bug_id = ?", undef, $clean_short_desc, $bug_id );
            }
        }
        print " done.\n" if $found;
    }
...
sub _stop_storing_inactive_flags {
    ...
    if ( $dbh->bz_column_info( 'flags', 'id' )->{'TYPE'} eq 'INT3' ) {
        ...
        $dbh->bz_alter_column( 'flags', 'id', { TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
    ...
    if ( $dbh->bz_column_info( 'bugs', 'short_desc' )->{TYPE} eq 'MEDIUMTEXT' ) {
        ...
        if (@$long_summary_bugs) {
            ...
            foreach my $bug (@$long_summary_bugs) {
                ...
                my $summary_comment = install_string( 'update_summary_truncate_comment', { summary => $summary } );
...
sub _add_classifications_sortkey {
    ...
    if ( !$dbh->bz_column_info( 'classifications', 'sortkey' ) ) {
        $dbh->bz_add_column( 'classifications', 'sortkey', { TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0 } );

        my $class_ids = $dbh->selectcol_arrayref('SELECT id FROM classifications ORDER BY name');
        my $sth       = $dbh->prepare( 'UPDATE classifications SET sortkey = ? ' . 'WHERE id = ?' );
        my $sortkey   = 0;
...
sub _move_data_nomail_into_db {
    ...
    $dbh->bz_add_column( 'profiles', 'disable_mail', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE' } );
    if ( -e "$datadir/nomail" ) {
        ...
        foreach my $user_to_check ( keys %nomail ) {
            my $uid
                = $dbh->selectrow_array( 'SELECT userid FROM profiles WHERE login_name = ?', undef, $user_to_check );
            next if !$uid;
...
sub _fix_uppercase_custom_field_names {
    ...
    my $dbh    = Bugzilla->dbh;
    my $fields = $dbh->selectall_arrayref('SELECT name, type FROM fielddefs WHERE custom = 1');
    foreach my $row (@$fields) {
        ...
        if ( $name ne lc($name) ) {
            ...
            $dbh->do( 'UPDATE fielddefs SET name = ? WHERE name = ?', undef, lc($name), $name );
        }
    }
}
...
sub _fix_uppercase_index_names {
    ...
    my $dbh    = Bugzilla->dbh;
    my $fields = $dbh->selectcol_arrayref( 'SELECT name FROM fielddefs WHERE type = ? AND custom = 1',
...
sub _initialize_workflow_for_upgrade {
    ...
    if ( !$num_closed_states ) {
        ...
        @closed_statuses = map { $dbh->quote($_) } ( @closed_statuses, qw(RESOLVED VERIFIED CLOSED) );

        print "Marking closed bug statuses as such...\n";
        $dbh->do( 'UPDATE bug_status SET is_open = 0 WHERE value IN (' . join( ', ', @closed_statuses ) . ')' );
    }
    ...
    my $status_id = $dbh->selectrow_array( 'SELECT id FROM bug_status WHERE value = ?', undef, $dup_status );
...
sub _fix_attachment_modification_date {
    ...
    if ( !$dbh->bz_column_info( 'attachments', 'modification_time' ) ) {
        ...
        $dbh->bz_alter_column( 'attachments', 'modification_time', { TYPE => 'DATETIME', NOTNULL => 1 } );
        ...
        my $attachments = $dbh->selectall_arrayref(
            'SELECT attach_id, MAX(bug_when) FROM bugs_activity
                                    WHERE attach_id IS NOT NULL ' . $dbh->sql_group_by('attach_id')
        );
    ...
    # patch omitting it.
    $dbh->bz_add_index( 'attachments', 'attachments_modification_time_idx', [qw(modification_time)] );
}

sub _change_text_types {
    my $dbh = Bugzilla->dbh;
    return
        if $dbh->bz_column_info( 'namedqueries', 'query' )->{TYPE} eq 'LONGTEXT';
    ...
    $dbh->bz_alter_column( 'longdescs',   'thetext',      { TYPE => 'LONGTEXT', NOTNULL => 1 } );
    $dbh->bz_alter_column( 'attachments', 'description',  { TYPE => 'TINYTEXT', NOTNULL => 1 } );
    $dbh->bz_alter_column( 'attachments', 'mimetype',     { TYPE => 'TINYTEXT', NOTNULL => 1 } );

    # This also changes NULL to NOT NULL.
    $dbh->bz_alter_column( 'flagtypes', 'description', { TYPE => 'MEDIUMTEXT', NOTNULL => 1 }, '' );
    $dbh->bz_alter_column( 'fielddefs', 'description', { TYPE => 'TINYTEXT',   NOTNULL => 1 } );
    $dbh->bz_alter_column( 'groups',    'description', { TYPE => 'MEDIUMTEXT', NOTNULL => 1 } );
    $dbh->bz_alter_column( 'quips',     'quip',        { TYPE => 'MEDIUMTEXT', NOTNULL => 1 } );
    $dbh->bz_alter_column( 'namedqueries', 'query', { TYPE => 'LONGTEXT', NOTNULL => 1 } );
...
sub _add_foreign_keys_to_multiselects {
    ...
    foreach my $name (@$names) {
        ...
        $dbh->bz_add_fk( "bug_$name", "value", { TABLE => $name, COLUMN => 'value', DELETE => 'RESTRICT' } );
    }
}
...
sub _populate_bugs_fulltext {
    my $bug_ids  = shift;
    my $dbh      = Bugzilla->dbh;
    my $fulltext = $dbh->selectrow_array( 'SELECT 1 FROM bugs_fulltext ' . $dbh->sql_limit(1) );
...
sub _add_visiblity_value_to_value_tables {
    ...
    my $custom_fields   = $dbh->selectcol_arrayref( 'SELECT name FROM fielddefs WHERE custom = 1 AND type IN(?,?)',
    ...
    foreach my $field ( @standard_fields, @$custom_fields ) {
        ...
        $dbh->bz_add_index( $field, "${field}_visibility_value_id_idx", ['visibility_value_id'] );
    }
}
...
sub _add_extern_id_index {
    ...
    if ( !$dbh->bz_index_info( 'profiles', 'profiles_extern_id_idx' ) ) {
        ...
        $dbh->bz_add_index( 'profiles', 'profiles_extern_id_idx', { TYPE => 'UNIQUE', FIELDS => [qw(extern_id)] } );
    }
}
...
sub _convert_disallownew_to_isactive {
    my $dbh = Bugzilla->dbh;
    if ( $dbh->bz_column_info( 'products', 'disallownew' ) ) {
        $dbh->bz_add_column( 'products', 'isactive', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'TRUE' } );
...
sub _fix_logincookies_ipaddr {
    ...
    $dbh->do( 'UPDATE logincookies SET ipaddr = NULL WHERE ipaddr = ?', undef, '0.0.0.0' );
}
...
sub _set_attachment_comment_type {
    ...
    my $test = $dbh->selectrow_array( "SELECT 1 FROM longdescs WHERE type = $type " . $dbh->sql_limit(1) );
    return [] if $test;
    my %comments = @{
        $dbh->selectcol_arrayref(
...
sub _set_attachment_comment_types {
    my $dbh = Bugzilla->dbh;
    $dbh->bz_start_transaction();
    my $created_ids = _set_attachment_comment_type( CMT_ATTACHMENT_CREATED, 'Created an attachment (id=' );
    my $updated_ids = _set_attachment_comment_type( CMT_ATTACHMENT_UPDATED, '(From update of attachment ' );
    $dbh->bz_commit_transaction();
    ...
    my $bug_ids = $dbh->selectcol_arrayref(
        'SELECT DISTINCT bug_id FROM longdescs WHERE ' . $dbh->sql_in( 'comment_id', \@comment_ids ) );
...
sub _remove_attachment_isurl {
    ...
    if ( $dbh->bz_column_info( 'attachments', 'isurl' ) ) {
        ...
        $dbh->do( 'UPDATE attachments SET filename = ? WHERE isurl = 1', undef, 'url.txt' );
...
sub _migrate_user_tags {
    ...
    my $sth_tag_id  = $dbh->prepare('SELECT id FROM tag WHERE user_id = ? AND name = ?');
...
sub _populate_bug_see_also_class {
    ...
    if ( $dbh->bz_column_info( 'bug_see_also', 'class' ) ) {
        ...
        $dbh->bz_alter_column( 'bug_see_also', 'class', { TYPE => 'varchar(255)', NOTNULL => 1, DEFAULT => "''" } );
        return;
    }

    $dbh->bz_add_column( 'bug_see_also', 'class', { TYPE => 'varchar(255)', NOTNULL => 1, DEFAULT => "''" }, '' );

    my $result = $dbh->selectall_arrayref("SELECT id, value FROM bug_see_also");

    my $update_sth = $dbh->prepare("UPDATE bug_see_also SET class = ? WHERE id = ?");
...
sub _migrate_disabledtext_boolean {
    ...
    if ( !$dbh->bz_column_info( 'profiles', 'is_enabled' ) ) {
        $dbh->bz_add_column( "profiles", 'is_enabled', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'TRUE' } );
        $dbh->do(
...
sub _rename_tags_to_tag {
    ...
    if ( $dbh->bz_table_info('tags') ) {
        ...
        $dbh->bz_add_index( 'tag', 'tag_user_id_idx', { FIELDS => [qw(user_id name)], TYPE => 'UNIQUE' } );
    }
...
sub _fix_notnull_defaults {
    ...
    my $custom_fields = Bugzilla::Field->match(
        {
            custom => 1,
            type   => [ FIELD_TYPE_FREETEXT, FIELD_TYPE_TEXTAREA ]
        }
    );
...
sub _fix_longdescs_primary_key {
    ...
    if ( $dbh->bz_column_info( 'longdescs', 'comment_id' )->{TYPE} ne 'INTSERIAL' ) {
        ...
        $dbh->bz_alter_column( 'longdescs', 'comment_id', { TYPE => 'INTSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
    }
}
        ...
        if ( $idx_info && $idx_info->{'TYPE'} ne 'UNIQUE' ) {
            ...
            my $dupes = $dbh->selectall_arrayref( "
                SELECT type_id, product_id, component_id, COUNT(*) AS count
                  FROM $table " . $dbh->sql_group_by('type_id, product_id, component_id') . "
                HAVING COUNT(*) > 1",
                { Slice => {} } );
...
sub _fix_user_api_keys_indexes {
    ...
    if ( $dbh->bz_index_info( 'user_api_keys', 'user_api_keys_key' ) ) {
        ...
        $dbh->bz_add_index( 'user_api_keys', 'user_api_keys_api_key_idx', { FIELDS => ['api_key'], TYPE => 'UNIQUE' } );
    }
...
sub _add_attach_size {
    ...
    $dbh->bz_add_column( 'attachments', 'attach_size', { TYPE => 'INT4', NOTNULL => 1, DEFAULT => 0 } );
...
sub _add_restrict_ipaddr {
    ...
    $dbh->bz_add_column( 'logincookies', 'restrict_ipaddr', { TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 0 } );
...
sub _migrate_preference_categories {
    ...
    $dbh->bz_add_column( 'setting', 'category', { TYPE => 'varchar(64)', NOTNULL => 1, DEFAULT => "'General'" } );
    my @settings = @{ Bugzilla::Install::SETTINGS() };
    foreach my $params (@settings) {
        $dbh->do( 'UPDATE setting SET category = ? WHERE name = ?', undef, $params->{category}, $params->{name} );
    }
}
