    ...
    foreach my $line ( @{ $section->{lines} } ) {
        ...
        if ( $line =~ /^[-\+]/ ) {
            ...
            if ( !exists( $this->{SECTION} ) ) {
                ...
                $this->push_context_lines( $this->{SECTION_END} + 1, $this->{NEXT_PATCH_LINE} - 1 );
            }
            else {
...
sub _start_section {
    ...
    $this->push_context_lines( $this->{SECTION}{old_start}, $this->{NEXT_PATCH_LINE} - 1 );
}
...
sub flush_section {
    ...
    if ( $this->{SECTION} ) {
        ...
        if ( $this->{CONTEXT} eq "file" ) {
            ...
            $this->push_context_lines( $this->{SECTION_END} + 1, $this->{SECTION_END} + $this->{CONTEXT} );
        }
