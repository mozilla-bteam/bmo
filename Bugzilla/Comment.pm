...
sub update {
    ...
    if ( @$removed_tags || @$added_tags ) {
        ...
        my $sth_delete   = $dbh->prepare( "DELETE FROM longdescs_tags WHERE comment_id = ? AND tag = ?" );
        my $sth_insert   = $dbh->prepare( "INSERT INTO longdescs_tags(comment_id, tag) VALUES (?, ?)" );
        ...
        foreach my $tag (@$removed_tags) {
            ...
            $sth_activity->execute( $self->bug_id, $self->id, Bugzilla->user->id, $when, '', $tag );
        }
...
sub attachment {
    ...
    $self->{attachment} ||= new Bugzilla::Attachment( { id => $self->extra_data, cache => 1 } );
...
sub add_tag {
    ...
    push @$tags, $tag;
    $self->{'tags'} = [ sort @$tags ];
    Bugzilla::Hook::process( "comment_after_add_tag", { comment => $self, tag => $tag } );
}
...
sub remove_tag {
    ...
    splice( @$tags, $index, 1 );
    Bugzilla::Hook::process( "comment_after_remove_tag", { comment => $self, tag => $tag } );
}
    ...
    if ( $type == CMT_NORMAL ) {
            ...
            or $type == CMT_ATTACHMENT_UPDATED )
        {
            ...
            detaint_natural($extra_data)
                or ThrowCodeError( 'comment_extra_data_not_numeric', { type => $type, extra_data => $original } );
        }
    }
...
sub _check_bug_id {
    ...
    ThrowCodeError(
        'param_required',
        {
            function => 'Bugzilla::Comment->create',
            param    => 'bug_id'
        }
    ) unless $bug_id;
