...
sub edit {
  ...
  if ($description ne $client->{desciption}) {
    $dbh->do('UPDATE oauth2_client SET description = ? WHERE id = ?',
      undef, $description, $id_old);
  }
  if ($secret ne $client->{secret}) {
    $dbh->do('UPDATE oauth2_client SET secret = ? WHERE id = ?',
      undef, $secret, $id_old);
  }
  ...
  if ($id_old ne $id) {
    $dbh->do('UPDATE oauth2_client SET id = ? WHERE id = ?',
      undef, $id, $id_old);
  }

  $dbh->do('DELETE FROM oauth2_client_scope WHERE client_id = ?', undef, $id);
  foreach my $scope (@{$all_scopes}) {
    if (first { $_ == $scope->{id} } @scopes) {
      $dbh->do(
        'REPLACE INTO oauth2_client_scope (client_id, scope_id, allowed) VALUES (?, ?, 1)',
        undef, $id, $scope->{id}
      );
    }
  }
