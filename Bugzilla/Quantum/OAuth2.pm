...
sub _store_access_token {
  ...
  my ($c, $client, $auth_code, $access_token, $refresh_token, $expires_in,
    $scope, $old_refresh_token)
    = @args{
    qw/ mojo_controller client_id auth_code access_token refresh_token expires_in scope old_refresh_token /
    };
  my $dbh = Bugzilla->dbh;
  my ($user_id);
...
sub _verify_access_token {
    ...
    )
  {
    ...
    my $access_token_data = $dbh->selectrow_hashref(
      'SELECT * FROM oauth2_access_token WHERE access_token = ?', undef,
      $access_token
    )
    )
  {
