...
sub user_map {
    my $self = shift;
    $self->{user_map} ||= { map { $_->{gnats_id} => $_->{login_name} } @{ $self->users } };
...
sub _get_gnats_field_data {
    ...
    my $originator = $self->translate_value( 'reporter', $fields{Originator}, { check_only => 1 } );
    if ( $originator !~ Bugzilla->params->{emailregexp} ) {
        ...
        if ($address) {
            $self->debug( "PR $fields{Number} had an Originator that was not a valid"
                    ...
                    . " instead.\n" );
            my $address_email = $self->translate_value( 'reporter', $address, { check_only => 1 } );
...
sub _parse_audit_trail {
    ...
    foreach my $line (@all_lines) {
        ...
        if ( $line =~ /^(\S+)-Changed-(\S+):(.*)/ ) {
            ...
            $self->debug( "Extra Audit-Trail line on $bug->{product} $bug->{bug_id}:" . " $line\n", 2 );
            $extra_comment .= "$line\n";
        }
    }
    $self->_store_audit_change( \@changes, $current_field, \%current_data );
...
sub _store_audit_change {
    ...
    $current_data->{removed} = $self->translate_value( $old_field, $current_data->{removed} );
    $current_data->{added}   = $self->translate_value( $old_field, $current_data->{added} );
...
END
    ...
    if ( $part1->body ) {
        $self->debug( " Additional Unformatted data found on " . $fields->{Category} . " bug " . $fields->{Number} );
    ...
    if ( scalar(@attachments) ne $num_attachments ) {
        warn "WARNING: Expected $num_attachments attachments but got " . scalar(@attachments) . "\n";
