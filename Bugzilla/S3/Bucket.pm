...
sub add_key {
    ...
    if ( ref($value) ) {
        return $self->account->_send_request_expect_nothing_probed( 'PUT', $self->_uri($key), $conf, $value );
    }
    else {
        return $self->account->_send_request_expect_nothing( 'PUT', $self->_uri($key), $conf, $value );
    }
}
...
sub delete_key {
    my ( $self, $key ) = @_;
    croak 'must specify key' unless $key && length $key;
    return $self->account->_send_request_expect_nothing( 'DELETE', $self->_uri($key), {} );
}
...
sub set_acl {
    ...
    return $self->account->_send_request_expect_nothing( 'PUT', $path, $hash_ref, $xml );
