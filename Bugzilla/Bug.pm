...
sub send_changes {
    ...
    my $recipient_count = _send_bugmail({ id => $self->id, type => 'bug', forced => \%forced,
    ...
    if ($new_dup_id) {
        $recipient_count += _send_bugmail({ forced => { changer => $user }, type => "dupe",
    ...
    foreach my $id (sort { $a <=> $b } (keys %changed_deps)) {
        $recipient_count += _send_bugmail({ forced => { changer => $user }, type => "dep",
    ...
    foreach my $ref_bug_id (uniq @{ $self->{see_also_changes} || [] }) {
        $recipient_count += _send_bugmail({ forced => { changer => $user },
