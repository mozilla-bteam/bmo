...
sub DB_COLUMNS {
    my $dbh = Bugzilla->dbh;
    my @custom
        = grep { $_->type != FIELD_TYPE_MULTI_SELECT } Bugzilla->active_custom_fields( { skip_extensions => 1 } );
...
sub ES_SELECT_UPDATED_SQL {
    ...
    my @fields = (
        'keywords',          'short_desc', 'product',    'component', 'cf_crash_signature', 'alias',
        'status_whiteboard', 'bug_status', 'resolution', 'priority',  'assigned_to'
    );
...
sub possible_duplicates {
    ...
    if ( $dbh->FULLTEXT_OR ) {
        ...
        ( $where_sql, $relevance_sql ) = $dbh->sql_fulltext_search( 'bugs_fulltext.short_desc', $joined_terms );
        $relevance_sql ||= $where_sql;
    }
    else {
        my ( @where, @relevance );
        foreach my $word (@words) {
            my ( $term, $rel_term ) = $dbh->sql_fulltext_search( 'bugs_fulltext.short_desc', $word );
            push( @where, $term );
    ...
    my $possible_dupes = $dbh->selectall_arrayref(
       ...
       ORDER BY relevance DESC, bug_id DESC " . $dbh->sql_limit($sql_limit), { Slice => {} }
    );
...
sub create {
    ...
    foreach my $field ( keys %$ms_values ) {
        ...
        foreach my $value ( @{ $ms_values->{$field} } ) {
            $dbh->do( "INSERT INTO bug_$field (bug_id, value) VALUES (?,?)", undef, $bug->bug_id, $value );
        }
    }
...
sub run_create_validators {
    ...
    my @mandatory_fields = @{ Bugzilla->fields(
            {
                is_mandatory => 1,
                enter_bug    => 1,
                obsolete     => 0
    ...
    ( $params->{dependson}, $params->{blocked} )
        = $class->_check_dependencies( $params->{dependson}, $params->{blocked}, $product );
...
sub update {
    ...
    if ( scalar @$removed_kw ) {
        $dbh->do( 'DELETE FROM keywords WHERE bug_id = ? AND ' . $dbh->sql_in( 'keywordid', $removed_kw ),
            undef, $self->id );
    }
    ...
    foreach my $comment ( @{ $self->{added_comments} || [] } ) {
        ...
        if ( $comment->work_time ) {
            LogActivityEntry( $self->id, "work_time", "", $comment->work_time, $user->id, $delta_ts );
        }
    ...
    foreach my $comment ( @{ $self->{comment_isprivate} || [] } ) {
        ...
        LogActivityEntry( $self->id, "longdescs.isprivate", $from, $to, $user->id, $delta_ts, $comment->id );
    }
    ...
    foreach my $field (@multi_selects) {
        ...
        if ( scalar @$removed || scalar @$added ) {
            ...
            foreach my $value ( @{ $self->$name } ) {
                $dbh->do( "INSERT INTO bug_$name (bug_id, value) VALUES (?,?)", undef, $self->id, $value );
            }
        }
    }
    ...
    my ( $removed_see, $added_see ) = diff_arrays( $old_bug->see_also, $self->see_also, 'name' );
    ...
    if ( scalar @$removed_see || scalar @$added_see ) {
        $changes->{see_also}
            = [ join( ', ', map { $_->name } @$removed_see ), join( ', ', map { $_->name } @$added_see ) ];
    }
...
sub _sync_fulltext {
    ...
    if ( $options{new_bug} || $options{update_comments} ) {
        my $comments
            = $dbh->selectall_arrayref( 'SELECT thetext, isprivate FROM longdescs WHERE bug_id = ?', undef, $self->id );
        $all_comments = join( "\n", map { $_->[0] } @$comments );
    ...
    if ( $options{new_bug} ) {
        ...
        if (@names) {
            $dbh->do( 'UPDATE bugs_fulltext SET ' . join( ', ', map {"$_ = ?"} @names ) . ' WHERE bug_id = ?',
...
sub _update_delta_ts {
    ...
    Bugzilla->dbh->do( "UPDATE bugs SET delta_ts = ? WHERE bug_id = ?", undef, $timestamp, $bug_id );
    Bugzilla::Hook::process( 'bug_end_of_update_delta_ts', { bug_id => $bug_id, timestamp => $timestamp } );
}
...
sub send_changes {
    ...
    push @results, _send_bugmail( { id => $self->id, type => 'bug', forced => \%forced } );

    # If the bug was marked as a duplicate, we need to notify users on the
    # other bug of any changes to that bug.
    my $new_dup_id = $changes->{'dup_id'} ? $changes->{'dup_id'}->[1] : undef;
    if ($new_dup_id) {
        push @results, _send_bugmail( { forced => { changer => $user }, type => "dupe", id => $new_dup_id } );
    }
    ...
    foreach my $id ( sort { $a <=> $b } ( keys %changed_deps ) ) {
        push @results, _send_bugmail( { forced => { changer => $user }, type => "dep", id => $id } );
    }
...
sub _send_bugmail {
    ...
    my $sent_bugmail = Bugzilla::BugMail::Send( $params->{'id'}, $params->{'forced'}, $params );
            ...
            || $user->in_group( 'canconfirm', $product->id ) )
        {
            ...
            unless ($new_status) {
                if ( scalar(@valid_statuses) == 1 ) {
                    ...
                    $new_status
                        = ( $valid_statuses[0]->name ne 'UNCONFIRMED' ) ? $valid_statuses[0] : $valid_statuses[1];
                }
            }
        }
        else {
...
sub _check_comment {
    ...
    $comment = Bugzilla::Comment->run_create_validators( $comment, { skip => ['bug_id'] } );
...
sub _check_keywords {
    ...
    foreach my $keyword ( values %keywords ) {
        ...
        ThrowUserError( 'value_inactive', { value => $keyword->name, class => ref $keyword } );
    }
...
sub _check_resolution {
        ...
        && scalar @{ $self->dependson } )
    {
        ...
        if ($count_open) {
            ThrowUserError( "still_unresolved_bugs", { bug_id => $self->id, dep_count => $count_open } );
        }
    }
...
sub _check_short_desc {
    ...
    if ( length($short_desc) > MAX_FREETEXT_LENGTH ) {
        ThrowUserError( 'freetext_too_long', { field => 'short_desc', text => $short_desc } );
    }
    return $short_desc;
}
...
sub _check_integer_field {
    ...
    if ( !detaint_signed($value) ) {
        ...
        ThrowUserError( "number_too_large", { field => $field, num => $orig_value, max_num => MAX_INT_32 } );
    }
...
sub _check_relationship_loop {
    ...
    my $dbh = Bugzilla->dbh;
    my $next_dep_id = $dbh->selectrow_array( "SELECT $field FROM bugs WHERE bug_id = ?", undef, $dep_id );
...
sub set_all {
    ...
    my @custom_fields = grep { $_->type != FIELD_TYPE_EXTENSION } Bugzilla->active_custom_fields;
...
sub set_dup_id {
    ...
    if ( $self->resolution ne 'DUPLICATE' ) {
        ...
        $self->set_bug_status( Bugzilla->params->{'duplicate_or_move_bug_status'}, { resolution => 'DUPLICATE' } );
    }
...
sub add_tag {
    ...
    if ( !$tag_id ) {
        $dbh->do( 'INSERT INTO tag (user_id, name) VALUES (?, ?)', undef, ( $user->id, $tag ) );
    ...
    $dbh->do( 'INSERT INTO bug_tag (bug_id, tag_id) VALUES (?, ?)', undef, ( $self->id, $tag_id ) );
...
sub remove_tag {
    ...
    $dbh->do( 'DELETE FROM bug_tag WHERE bug_id = ? AND tag_id = ?', undef, ( $self->id, $tag_id ) );
    ...
    if ( !--$user->tags->{$tag}->{bug_count} ) {
        $dbh->do( 'DELETE FROM tag WHERE name = ? AND user_id = ?', undef, ( $tag, $user->id ) );
...
sub attachments {
    ...
    $self->{'attachments'} = Bugzilla::Attachment->get_attachments_by_bug( $self, { preload => 1 } );
...
sub component_obj {
    ...
    return {} if $self->{error};
    $self->{component_obj} = new Bugzilla::Component( { id => $self->{component_id}, cache => 1 } );
...
sub classification_id {
    ...
    ( $self->{classification_id} )
        = Bugzilla->dbh->selectrow_array( 'SELECT classification_id FROM products WHERE id = ?',
...
sub classification {
    ...
    return '' if $self->{error};
    ( $self->{classification} ) = Bugzilla->dbh->selectrow_array( 'SELECT name FROM classifications WHERE id = ?',
...
sub duplicate_ids {
    ...
    my $dbh = Bugzilla->dbh;
    $self->{duplicate_ids}
        = $dbh->selectcol_arrayref( 'SELECT dupe FROM duplicates WHERE dupe_of = ?', undef, $self->id );
...
sub keyword_objects {
    ...
    my $dbh = Bugzilla->dbh;
    my $ids = $dbh->selectcol_arrayref( "SELECT keywordid FROM keywords WHERE bug_id = ?", undef, $self->id );
...
sub product_obj {
    my $self = shift;
    return {} if $self->{error};
    $self->{product_obj} ||= new Bugzilla::Product( { id => $self->{product_id}, cache => 1 } );
...
sub see_also {
    ...
    if ( !exists $self->{see_also} ) {
        my $ids = Bugzilla->dbh->selectcol_arrayref( 'SELECT id FROM bug_see_also WHERE bug_id = ?', undef, $self->id );
...
sub groups {
    ...
    foreach my $row (@$rows) {
        my ( $groupid, $name, $description, $ison, $ingroup, $isactive, $membercontrol, $othercontrol ) = @$row;
...
sub groups_in {
    ...
    return [] if $self->{'error'};
    my $group_ids
        = Bugzilla->dbh->selectcol_arrayref( 'SELECT group_id FROM bug_group_map WHERE bug_id = ?', undef, $self->id );
...
sub choices {
    ...
    my $classifications = Bugzilla::Classification->new_from_list( [ keys %class_ids ] );
...
sub EmitDependList {
    ...
    return $dbh->selectcol_arrayref( $cache->{"${target_field}_sth_$exclude_resolved"}, undef, $bug_id );
}
...
sub LogActivityEntry {
    my ( $i, $col, $removed, $added, $whoid, $timestamp, $comment_id, $attach_id ) = @_;
    my $dbh = Bugzilla->dbh;
...
sub check_can_change_field {
    ...
    elsif ( ( $field eq 'estimated_time' || $field eq 'remaining_time' || $field eq 'work_time' )
        && $oldvalue == $newvalue )
    {
        return 1;
    }
...
sub _multi_select_accessor {
    ...
    my $accessor = sub {
        my ($self) = @_;
        $self->{$field}
            ||= Bugzilla->dbh->selectcol_arrayref( "SELECT value FROM bug_$field WHERE bug_id = ? ORDER BY value",
            undef, $self->id );
        return $self->{$field};
    };
    return $accessor;
