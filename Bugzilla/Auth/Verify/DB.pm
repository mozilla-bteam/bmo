...
sub check_credentials {
        ...
        && Bugzilla->params->{password_check_on_login} )
    {
        ...
        unless ( $pwqc->validate_password($password) ) {
            ...
            $user->set_password_change_reason( "You must change your password for the following reason: $reason" );
            $user->update();
        }
    }
...
sub change_password {
    ...
    $dbh->do( "UPDATE profiles SET cryptpassword = ? WHERE userid = ?", undef, $cryptpassword, $user->id );
