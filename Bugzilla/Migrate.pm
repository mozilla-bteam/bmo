...
sub do_migration {
    ...
    if ( $self->config('starting_bug_id') ) {
        $dbh->bz_set_next_serial_value( 'bugs', 'bug_id', $self->config('starting_bug_id') );
    }
...
sub check_requirements {
    my $self    = shift;
    my $missing = Bugzilla::Install::Requirements::_check_missing( $self->REQUIRED_MODULES, 1 );
    my %results = (
        apache      => [],
        pass        => @$missing ? 0 : 1,
        missing     => $missing,
...
sub translate_value {
    ...
    if (   $field eq 'creation_ts'
        ...
        or ($field_obj
            and (  $field_obj->type == FIELD_TYPE_DATETIME
...
sub write_config {
    ...
    foreach my $var ( $self->CONFIG_VARS ) {
        print $fh "\n", $var->{desc}, Data::Dumper->Dump( [ $var->{default} ], [ $var->{name} ] );
    }
    close($fh);
}
...
sub insert_products {
    ...
    foreach my $product (@$products) {
        ...
        foreach my $component (@$components) {
            next if new Bugzilla::Component(
                {
                    product => $created_prod,
                    name    => $component->{name}
                }
            );
            my $created_comp = Bugzilla::Component->create( { %$component, product => $created_prod } );
            print '  ',
                get_text(
                'migrate_component_created',
                {
                    comp    => $created_comp,
                    product => $created_prod
                }
...
sub create_legal_values {
    ...
    foreach my $product ( keys %product_values ) {
        ...
        foreach my $version ( keys %{ $product_values{$product}->{version} } ) {
            next if new Bugzilla::Version(
                {
                    product => $prod_obj,
                    name    => $version
                }
            );
            my $created = Bugzilla::Version->create(
                {
                    product => $prod_obj,
                    value   => $version
                }
        ...
        foreach my $milestone ( keys %{ $product_values{$product}->{target_milestone} } ) {
            next if new Bugzilla::Milestone(
                {
                    product => $prod_obj,
                    name    => $milestone
                }
            );
            my $created = Bugzilla::Milestone->create( { product => $prod_obj, value => $milestone } );
...
sub insert_bugs {
    ...
    my @standard_drop_downs = grep { !$_->custom and $_->is_select } ( values %{ $self->bug_fields } );
    ...
    foreach my $bug (@$bugs) {
        ...
        if ( defined $bug->{resolution} ) {
            $set_resolution = Bugzilla::Field::Choice->type('resolution')->new( { name => delete $bug->{resolution} } );
        }
        ...
        if ( defined $bug->{delta_ts} ) {
            $dbh->do( 'UPDATE bugs SET delta_ts = ? WHERE bug_id = ?', undef, $bug->{delta_ts}, $created->id );
        }

        # We don't need to send email for imported bugs.
        $dbh->do( 'UPDATE bugs SET lastdiffed = delta_ts WHERE bug_id = ?', undef, $created->id );

        # We don't use set_ and update() because that would create
        # a bugs_activity entry that we don't want.
        if ($set_status) {
            $dbh->do( 'UPDATE bugs SET bug_status = ? WHERE bug_id = ?', undef, $set_status->name, $created->id );
        }
        if ($set_resolution) {
            $dbh->do( 'UPDATE bugs SET resolution = ? WHERE bug_id = ?', undef, $set_resolution->name, $created->id );
        }
...
sub _insert_history {
    ...
    foreach my $item (@$history) {
        ...
        LogActivityEntry( $bug->id, $item->{field}, $item->{removed}, $item->{added}, $who->id, $item->{bug_when} );
...
sub _insert_attachments {
    ...
    foreach my $attachment (@$attachments) {
        ...
        $self->debug( '  Attachment ' . $created->description . ' from ' . $submitter->login, 2 );
        Bugzilla->set_user($super_user);
    }
}
...
sub _do_table_insert {
    ...
    Bugzilla->dbh->do( "INSERT INTO $table ($field_sql) VALUES ($question_sql)", undef, @values );
}
