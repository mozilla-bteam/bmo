...
sub initFromParameters {
    ...
    (   $self->{'series_id'},   $self->{'category'},  $self->{'subcategory'}, $self->{'name'},
        $self->{'creator_id'},  $self->{'frequency'}, $self->{'query'},       $self->{'public'},
...
sub initFromCGI {
    ...
    $self->{'query'} = $cgi->canonicalise_query( "format", "ctype", "action", "category", "subcategory", "name",
...
sub writeToDatabase {
    ...
    if ($exists) {
        ...
        # Retrieve series_id
        $self->{'series_id'} = $dbh->selectrow_array( "SELECT MAX(series_id) " . "FROM series" );
        $self->{'series_id'}
...
sub getCategoryID {
    ...
    while (1) {
        ...
        $category_id = $dbh->selectrow_array(
            "SELECT id " . "from series_categories " . "WHERE name =" . $dbh->quote($category) );

        last if defined($category_id);

        $dbh->do( "INSERT INTO series_categories (name) " . "VALUES (" . $dbh->quote($category) . ")" );
    }
