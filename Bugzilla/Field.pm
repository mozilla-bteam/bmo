...
sub visibility_field {
    ...
    if ( $self->{visibility_field_id} ) {
        $self->{visibility_field} ||= $self->new( { id => $self->{visibility_field_id}, cache => 1 } );
    }
...
sub visibility_values {
    ...
    if ( !defined $self->{visibility_values} ) {
        ...
        $self->{visibility_values}
            = Bugzilla::Field::Choice->type( $self->visibility_field )->new_from_list($visibility_value_ids);
    }
...
sub controls_visibility_of {
    my $self = shift;
    $self->{controls_visibility_of} ||= Bugzilla::Field->match( { visibility_field_id => $self->id } );
...
sub controls_values_of {
    my $self = shift;
    $self->{controls_values_of} ||= Bugzilla::Field->match( { value_field_id => $self->id } );
    ...
    if ( $old_field_id && ( $old_field_name ne $new_field_name ) ) {
        ...
        my $broken_named_queries = $dbh->selectall_arrayref(
            'SELECT userid, name, query
                                        FROM namedqueries WHERE ' . $dbh->sql_istrcmp( 'query', '?', 'LIKE' ),
        ...
        my $broken_series = $dbh->selectall_arrayref(
            'SELECT series_id, query
                                        FROM series WHERE ' . $dbh->sql_istrcmp( 'query', '?', 'LIKE' ),
            undef, "%=$old_field_name%"
        );
