...
sub DB_COLUMNS {
    ...
    return qw(
        ...
        $dbh->sql_date_format( 'modification_date', '%Y.%m.%d %H:%i:%s' ) . ' AS modification_date';
}
...
sub set_flag {
    ...
    $bug->check_can_change_field( 'flagtypes.name', 0, 1, \$privs )
        || ThrowUserError( 'illegal_change', { field => 'flagtypes.name', privs => $privs } );
    ...
    if ( $params->{id} ) {
        ...
        ( $obj_flag, $requestee_changed ) = $class->_validate( $obj_flag, $obj_flagtype, $params, $bug, $attachment );
    }
    ...
    # Create a new flag.
        ...
        ( $obj_flag, $requestee_changed ) = $class->_validate( undef, $obj_flagtype, $params, $bug, $attachment );
    }
    else {
        ThrowCodeError(
            'param_required',
...
sub update {
    ...
    if ( scalar( keys %$changes ) ) {
        $dbh->do( 'UPDATE flags SET modification_date = ? WHERE id = ?', undef, ( $timestamp, $self->id ) );
        $self->{'modification_date'} = format_time( $timestamp, '%Y.%m.%d %T', Bugzilla->local_timezone );
...
sub _set_requestee {
    ...
    $self->{requestee} = $self->_check_requestee( $requestee, $bug, $attachment, $skip_requestee_on_error );
...
sub notify {
    ...
    foreach my $to ( keys %recipients ) {
        ...
        my $comments = $recipients{$to} && $recipients{$to}->is_insider ? $all_comments : $public_comments;
