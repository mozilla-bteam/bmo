...
sub COLUMN_JOINS {
    ...
    my $joins = {
        actual_time => {
            table => '(SELECT bug_id, SUM(work_time) AS total' . ' FROM longdescs GROUP BY bug_id)',
            join  => 'INNER',
        },
        assigned_to => {
            from  => 'assigned_to',
...
sub COLUMNS {
    ...
    my %special_sql = (
        ...
        'flagtypes.name' =>
            $dbh->sql_group_concat( 'DISTINCT ' . $dbh->sql_string_concat( 'map_flagtypes.name', 'map_flags.status' ) ),
...
sub REPORT_COLUMNS {
    ...
    my @multi_selects = @{ Bugzilla->fields( { obsolete => 0, type => FIELD_TYPE_MULTI_SELECT } ) };
...
sub _sql_order_by {
    ...
    if ( !$self->{sql_order_by} ) {
        my @order_by = map { $self->_translate_order_by_column($_) } $self->_valid_order;
        ...
        if ($others_like_me) {
            ...
            if ($from) {
                all { $_->{from} eq $from } @$others_like_me
                    or die "Not all same-named joins have identical 'from': " . Dumper( $join, $others_like_me );
            }
            if ($to) {
                all { $_->{to} eq $to } @$others_like_me
                    or die "Not all same-named joins have identical 'to': " . Dumper( $join, $others_like_me );
            }
...
sub _standard_joins {
    ...
    if ( $user->id ) {
        $security_join->{extra} = [ "NOT (" . $user->groups_in_sql('security_map.group_id') . ")" ];
...
sub _translate_join {
    ...
    my @join_sql = "$join JOIN $table AS $name" . " ON $from_table.$from = $name.$to$extra_condition";
    return @join_sql;
}
...
sub do_search_function {
    ...
    if ( !defined $args->{term} ) {
        ...
        ThrowUserError( "search_field_operator_invalid", { field => $field, operator => $operator } );
    }
}
...
sub _do_operator_function {
    ...
    my $operator_func = OPERATORS->{$operator}
        || ThrowCodeError( "search_field_operator_unsupported", { operator => $operator } );
...
sub _get_column_joins {
    ...
    Bugzilla::Hook::process( 'buglist_column_joins', { column_joins => \%column_joins, search => $self } );
...
sub _substring_terms {
    ...
    my @terms = map { $dbh->sql_iposition( $_, $args->{full_field} ) . " > 0" } @words;
    return @terms;
}
...
sub _contact_exact_group {
    my ( $self, $args ) = @_;
    my ( $value, $operator, $field, $chart_id, $joins ) = @$args{qw(value operator field chart_id joins)};
    my $dbh  = Bugzilla->dbh;
    my $user = $self->_user;
...
sub _cc_exact_group {
    my ( $self, $args ) = @_;
    my ( $chart_id, $sequence, $joins, $operator, $value ) = @$args{qw(chart_id sequence joins operator value)};
    my $user = $self->_user;
    my $dbh  = Bugzilla->dbh;
    ...
    my $group_join  = {
        ...
        from  => "$cc_table.who",
        to    => 'user_id',
        extra => [ $dbh->sql_in( "$group_table.group_id", $all_groups ), "$group_table.isbless = 0" ],
    };
...
sub _user_nonchanged {
    my ( $self, $args ) = @_;
    my ( $field, $operator, $chart_id, $sequence, $joins ) = @$args{qw(field operator chart_id sequence joins)};
    ...
    if ( my $join = USER_FIELDS->{$field}->{join} ) {
        ...
        my $extra = ( $isprivate and !$self->_user->is_insider ) ? ["$as.isprivate = 0"] : [];
...
sub _long_desc_changedbefore_after {
    my ( $self, $args ) = @_;
    my ( $chart_id, $operator, $value, $joins ) = @$args{qw(chart_id operator value joins)};
    my $dbh = Bugzilla->dbh;
...
sub _long_desc_nonchanged {
    my ( $self, $args ) = @_;
    my ( $chart_id, $operator, $value, $joins, $bugs_table ) = @$args{qw(chart_id operator value joins bugs_table)};
    ...
    # private comments
    if ( !$self->_user->is_insider ) {
        $join_args->{term} .= ( $join_args->{term} ? " AND " : "" ) . "$table.isprivate = 0";
    }
...
sub _content_matches {
    my ( $self, $args ) = @_;
    my ( $chart_id, $joins, $fields, $operator, $value ) = @$args{qw(chart_id joins fields operator value)};
    my $dbh = Bugzilla->dbh;
    ...
    my ( $term1, $rterm1 ) = $dbh->sql_fulltext_search( "$table.$comments_col", $value );
...
sub _long_descs_count {
    ...
    my $join  = {
        table => "(SELECT bug_id, COUNT(*) AS num" . " FROM longdescs $extra GROUP BY bug_id)",
        as    => $table,
    };
    push( @$joins, $join );
...
sub _days_elapsed {
    ...
    $args->{full_field} = "(" . $dbh->sql_to_days('NOW()') . " - " . $dbh->sql_to_days('bugs.delta_ts') . ")";
}
...
sub _component_nonchanged {
    ...
    $self->_do_operator_function($args);
    my $term = $args->{term};
    $args->{term} = build_subselect( "bugs.component_id", "components.id", "components", $args->{term} );
}
...
sub _product_nonchanged {
    ...
    $self->_do_operator_function($args);
    my $term = $args->{term};
    $args->{term} = build_subselect( "bugs.product_id", "products.id", "products", $term );
}
...
sub _classification_nonchanged {
    ...
    $self->_do_operator_function($args);
    my $term = $args->{term};
    $args->{term} = build_subselect( "map_product.classification_id", "classifications.id", "classifications", $term );
}
...
sub _owner_idle_time_greater_less {
    my ( $self, $args ) = @_;
    my ( $chart_id, $joins, $value, $operator ) = @$args{qw(chart_id joins value operator)};
    my $dbh = Bugzilla->dbh;
...
sub _multiselect_multiple {
    my ( $self, $args ) = @_;
    my ( $chart_id, $field, $operator, $value ) = @$args{qw(chart_id field operator value)};
    my $dbh = Bugzilla->dbh;
...
sub _multiselect_nonchanged {
    my ( $self, $args ) = @_;
    my ( $chart_id, $joins, $field, $operator ) = @$args{qw(chart_id joins field operator)};
...
sub _multiselect_table {
    ...
    if ( $field eq 'keywords' ) {
        ...
        return "keywords INNER JOIN keyworddefs" . " ON keywords.keywordid = keyworddefs.id";
    }
    elsif ( $field eq 'tag' ) {
        ...
        return "longdescs INNER JOIN longdescs_tags" . " ON longdescs.comment_id = longdescs_tags.comment_id";
    }
    my $table = "bug_$field";
...
sub _changedbefore_changedafter {
    my ( $self, $args ) = @_;
    my ( $chart_id, $joins, $field, $operator, $value ) = @$args{qw(chart_id joins field operator value)};
    my $dbh = Bugzilla->dbh;
    ...
    if ( $field_object->name eq 'creation_ts' ) {
        $args->{operator} = $operator eq 'changedbefore' ? 'lessthaneq' : 'greaterthaneq';
    ...
    my $join     = {
        table => 'bugs_activity',
        as    => $table,
        extra => [ "$table.fieldid = $field_id", "$table.bug_when $sql_operator $sql_date" ],
    };
...
sub _changedfrom_changedto {
    my ( $self, $args ) = @_;
    my ( $chart_id, $joins, $field, $operator, $quoted ) = @$args{qw(chart_id joins field operator quoted)};
