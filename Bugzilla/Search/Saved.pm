...
sub new {
    ...
    if ( ref $param ) {
        ...
        detaint_natural($user_id)
            || ThrowCodeError( 'param_must_be_numeric', { function => $class . '::_init', param => 'user' } );
...
sub shared_with_group {
    ...
    my ($group_id)
        = Bugzilla->dbh->selectrow_array( 'SELECT group_id FROM namedquery_group_map WHERE namedquery_id = ?',
        undef, $self->id );
    $self->{shared_with_group}
        = $group_id
        ? new Bugzilla::Group($group_id)
        : undef;
...
sub user {
    my ($self) = @_;
    return $self->{user} ||= Bugzilla::User->new( { id => $self->{userid}, cache => 1 } );
}
