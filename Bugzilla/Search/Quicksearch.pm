...
sub quicksearch {
    ...
    if ( $searchstring =~ m/^[0-9,\s]*$/ ) {
        ...
        foreach my $qsword (@qswords) {
            ...
            foreach my $term (@or_operand) {
                ...
                next if _handle_field_names( $term, $negate, \@unknownFields, \%ambiguous_fields );
    ...
    if ( $cgi->param('load') ) {
        ...
        print $cgi->redirect( -uri => "${urlbase}query.cgi?format=advanced&amp;" . $modified_query_string );
    }
...
sub _bug_numbers_only {
    ...
    if ( $searchstring !~ /,/ && !i_am_webservice() ) {
        ...
        print $cgi->redirect( -uri => correct_urlbase() . "show_bug.cgi?id=$searchstring" );
        exit;
    }
    else {
        # List of bug numbers.
...
sub _handle_alias {
    ...
    if ( $searchstring =~ /^([^,\s]+)$/ ) {
        ...
        my $bug_id = Bugzilla->dbh->selectrow_array( q{SELECT bug_id FROM bugs WHERE alias = ?}, undef, $alias );
