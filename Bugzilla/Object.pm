...
sub _load_from_db {
    ...
    if ( defined $id ) {
        ...
        detaint_natural($id)
            || ThrowCodeError( 'param_must_be_numeric', { function => $class . '::_load_from_db' } );
        ...
        $object_data = $dbh->selectrow_hashref( "SELECT $columns FROM $table WHERE $condition", undef, @values );
    }
    return $object_data;
}
...
sub new_from_list {
    ...
    foreach my $id (@$id_list) {
        detaint_natural($id)
            || ThrowCodeError( 'param_must_be_numeric', { function => $class . '::new_from_list' } );
...
sub set_all {
    ...
    Bugzilla::Hook::process( 'object_end_of_set_all', { object => $self, params => \%field_values } );
}
...
sub update {
    ...
    Bugzilla::Hook::process( 'object_start_of_update', { object => $self, old_object => $old_self } );

    my @all_columns = $self->UPDATE_COLUMNS;
    my @hook_columns;
    Bugzilla::Hook::process( 'object_update_columns', { object => $self, columns => \@hook_columns } );
...
sub run_create_validators {
    ...
    Bugzilla::Hook::process( 'object_end_of_create_validators', { class => $class, params => \%field_values } );
...
sub insert_create_data {
    ...
    chop($qmarks);
    my $table = $class->DB_TABLE;
    $dbh->do( "INSERT INTO $table (" . join( ', ', @field_names ) . ") VALUES ($qmarks)", undef, @values );
...
sub _insert_dep_field {
    ...
    foreach my $required_field (@$required_fields) {
        ...
        _insert_dep_field( $required_field, $insert_me, $dependencies, $result, $loop_tracking );
    }
    push( @$result, $field );
}
...
sub _get_db_columns {
    ...
    my @add_columns;
    Bugzilla::Hook::process( 'object_columns', { class => $class, columns => \@add_columns } );
...
sub _get_validators {
    ...
    Bugzilla::Hook::process( 'object_validators', { class => $class, validators => \%validators } );
