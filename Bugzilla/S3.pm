...
__PACKAGE__->mk_accessors( qw(aws_access_key_id aws_secret_access_key secure ua err errstr timeout retry host) );
our $VERSION = '0.45bmo';
...
sub _validate_acl_short {
    ...
    if ( !grep( { $policy_name eq $_ } qw(private public-read public-read-write authenticated-read) ) ) {
