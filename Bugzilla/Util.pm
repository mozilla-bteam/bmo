...
sub template_var {
    ...
    my $result = $template->process( 'global/field-descs.none.tmpl', { vars => \%vars, in_template_var => 1 } );
...
sub detect_encoding {
    ...
    if ( !Bugzilla->feature('detect_charset') ) {
        require Bugzilla::Error;
        Bugzilla::Error::ThrowCodeError( 'feature_disabled', { feature => 'detect_charset' } );
    }
