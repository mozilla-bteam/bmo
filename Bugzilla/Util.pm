...
sub bz_crypt {
    my ($password, $stored_password) = @_;
    if (Bugzilla->has_feature('argon2')) {
        require Bugzilla::Password::Argon2id;
    }
    require Bugzilla::Password::Digest;
    require Bugzilla::Password::Crypt;

    if (defined $stored_password) {
        if ( $stored_password =~ /{([^}]+)}$/ ) {
            my $salt = substr $stored_password, 0, PASSWORD_SALT_LENGTH;
            return Bugzilla::Password::Digest->new(
                salt      => $salt,
                password  => $password,
                algorithm => $1,
            );
        }
        elsif ($stored_password =~ /^\$argon2id\$/) {
            die 'Cannot handle argon2 passwords!' unless Bugzilla->has_feature('argon2');
            return Bugzilla::Password::Argon2id->new(
                salt     => generate_random_password(PASSWORD_SALT_LENGTH * 2),
                password => $password,
            );
        }
        else {
            my $salt = substr $stored_password, 0, PASSWORD_SALT_LENGTH;
            return Bugzilla::Password::Crypt->new(
                salt      => $salt,
                password  => $password,
            );
        }
