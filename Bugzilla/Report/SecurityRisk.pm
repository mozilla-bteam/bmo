...
use POSIX qw(ceil);
use Type::Utils;
use Types::Standard qw(Num Int Bool Str HashRef ArrayRef CodeRef Map Dict Enum Object);
...
sub _build_results {
    ...
    for ( my $report_date = $self->end_date->clone(); $report_date >= $self->start_date; $report_date->subtract( weeks => 1 ) ) {
