...
sub _bug_to_hash {
  ...
  if (defined $params->{include_fields} && !defined $params->{exclude_fields}) {
    $field_params->{name}
      = ref $params->{include_fields}
      ? $params->{include_fields}
      : [split(/[\s,]+/, $params->{include_fields})];
  }
  my @custom_fields = Bugzilla->active_custom_fields($field_params);
