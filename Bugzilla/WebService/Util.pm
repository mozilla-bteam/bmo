...
sub extract_flags {
    ...
    foreach my $flag (@$flags_copy) {
        ...
        if ($id) {
            ...
            $flag_obj || ThrowUserError( 'object_does_not_exist', { class => 'Bugzilla::Flag', id => $id } );
        }
        elsif ($type_id) {
            my $type_obj = grep( $type_id == $_->id, @$flag_types );
            $type_obj || ThrowUserError( 'object_does_not_exist', { class => 'Bugzilla::FlagType', id => $type_id } );
            if ( !$new ) {
            ...
            @type_matches
                || ThrowUserError( 'object_does_not_exist', { class => 'Bugzilla::FlagType', name => $name } );
            if ($new) {
                delete $flag->{id};
...
sub params_to_objects {
    ...
    @objects_by_ids = map { $class->check( { id => $_ } ) } @{ $params->{ids} } if $params->{ids};
