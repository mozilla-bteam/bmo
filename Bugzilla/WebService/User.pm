...
sub get {
    ...
    Bugzilla->switch_to_shadow_db();
        ...
        || ThrowCodeError( 'params_required', { function => 'User.get', params => [ 'ids', 'names', 'match' ] } );
    ...
    if ( $params->{limit} ) {
        detaint_natural( $params->{limit} )
            || ThrowCodeError( 'param_must_be_numeric', { function => 'User.match', param => 'limit' } );
    ...
    Bugzilla::Hook::process( 'webservice_user_get', { webservice => $self, params => $params, users => \@users } );
...
sub update {
    ...
    defined( $params->{names} )
        || defined( $params->{ids} )
        || ThrowCodeError( 'params_required', { function => 'User.update', params => [ 'ids', 'names' ] } );
