...
sub handle {
    ...
    if ( !$self->error_response_header ) {
        return $self->response( $self->response_header( $self->bz_success_code || STATUS_OK, $result ) );
    }
...
sub response {
    ...
    Bugzilla::Hook::process( 'webservice_rest_response', { rpc => $self, result => \$result, response => $response } );
...
sub _argument_type_check {
    ...
    Bugzilla::Hook::process( 'webservice_rest_request', { rpc => $self, params => $params } );
...
sub _find_resource {
    ...
    Bugzilla::Hook::process( 'webservice_rest_resources', { rpc => $self, resources => $resources } );
...
sub _get_content_prefs {
    ...
    @prefs = map { $_->{name} } sort { $b->{score} <=> $a->{score} || $a->{order} <=> $b->{order} } @prefs;
    return @prefs;
}
