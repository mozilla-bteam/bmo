...
if ( $cgi->param('nukedefaultquery') ) {
    if ($userid) {
        ...
        my $named_queries = Bugzilla::Search::Saved->match( { userid => $userid, name => DEFAULT_QUERY_NAME } );
        if (@$named_queries) {
...
my $userdefaultquery;
if ($userid) {
    $userdefaultquery = $dbh->selectrow_array( "SELECT query FROM namedqueries " . "WHERE userid = ? AND name = ?",
...
my @selectable_products = sort { lc( $a->name ) cmp lc( $b->name ) } @{ $user->get_selectable_products };
...
if ($userid) {
    $vars->{'namedqueries'}
        ...
        "SELECT name FROM namedqueries " . "WHERE userid = ? AND name != ? " . "ORDER BY name",
        undef, ( $userid, DEFAULT_QUERY_NAME ) );
}
...
if ( ( $cgi->param('query_format') || $cgi->param('format') || "" ) eq "create-series" ) {
...
# format.
my $format = $template->get_format( "search/search", $vars->{'query_format'} || $vars->{'format'},
