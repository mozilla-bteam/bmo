...
my $format = $template->get_format( "list/list", scalar $cgi->param('format'), scalar $cgi->param('ctype') );
...
sub LookupNamedQuery {
    ...
    my $query = Bugzilla::Search::Saved->check( { user => $sharer_id, name => $name, _error => 'missing_query' } );
...
sub LookupSeries {
    ...
    my $dbh = Bugzilla->dbh;
    my $result = $dbh->selectrow_array( "SELECT query FROM series " . "WHERE series_id = ?", undef, ($series_id) );
    $result
...
sub GetQuip {
    ...
    my $count  = $dbh->selectrow_array( "SELECT COUNT(quip)" . " FROM quips WHERE approved = 1" );
    my $random = int( rand($count) );
    my $quip   = $dbh->selectrow_array( "SELECT quip FROM quips WHERE approved = 1 " . $dbh->sql_limit( 1, $random ) );
    return $quip;
}
...
if ( $cmdtype eq "dorem" ) {
    if ( $remaction eq "run" ) {
        ...
        ( $buffer, $query_id, $sharer_id )
            = LookupNamedQuery( scalar $cgi->param("namedcmd"), scalar $cgi->param('sharer_id') );
        ...
        my $query_id;
        ( $buffer, $query_id ) = LookupNamedQuery( scalar $cgi->param("namedcmd"), $user->id );
        if ($query_id) {
    ...
    if ( $cgi->param('remtype') eq "asdefault" ) {
        ...
        $buffer = $params->canonicalise_query( 'cmdtype', 'remtype', 'query_based_on', 'token' );
...
my $time_info = {
    ...
    'time_present'        => ( $estimated_time || $remaining_time || $actual_time || $percentage_complete ),
};
...
foreach my $row (@$data) {
    ...
    if ( $bug->{'changeddate'} ) {
        $bug->{'changeddate'} =~ s/^(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})$/$1-$2-$3 $4:$5:$6/;
...
if (@bugidlist) {
    my $sth
        = $dbh->prepare( "SELECT DISTINCT bugs.bug_id, MIN(group_control_map.membercontrol) "
            . "FROM bugs "
...
if ( $dotweak && scalar @bugs ) {
    ...
    if ($one_product) {
        ...
        if ( Bugzilla->params->{'usetargetmilestone'} ) {
            $vars->{'targetmilestones'} = [ map( $_->name, grep( $_->is_active, @{ $one_product->milestones } ) ) ];
        }
    }
}
...
if ( $format->{'extension'} eq "html" && !$agent ) {
    ...
    my $search
        = $user->save_last_search( { bugs => \@bugidlist, order => $order, vars => $vars, list_id => $list_id } );
...
$vars->{'urlquerypart'} = $params->canonicalise_query( 'order', 'cmdtype', 'query_based_on', 'token' );
