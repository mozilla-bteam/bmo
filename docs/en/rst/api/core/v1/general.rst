...
Servers may reject a request when its URL or query string is too long. The limit
depends on the server configuration, and the response may be a non-JSON
``414 URI Too Long`` error generated by a front end instead of the JSON error
object described above. Keep request URLs conservatively sized and split large
queries into multiple requests.
